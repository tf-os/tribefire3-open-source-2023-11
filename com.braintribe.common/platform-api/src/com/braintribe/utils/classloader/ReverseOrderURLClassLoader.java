...
public class ReverseOrderURLClassLoader extends URLClassLoader {
	...
	public ReverseOrderURLClassLoader(URL[] urls, ClassLoader parent, Predicate<String> doParentFirst) {
		super(urls, parent);
		this.doParentFirst = doParentFirst;
	...
	protected Class<?> loadWithJavaPlatformClassLoaderIfRelevant(String name, boolean resolve) {
		...
		try {
			/* Use getResource as it won't trigger an expensive ClassNotFoundException if the resource is not available from the
			 * Java SE class loader. However (see https://bz.apache.org/bugzilla/show_bug.cgi?id=58125 for details) when running
			 * under a security manager in rare cases this call may trigger a ClassCircularityError. See
			 * https://bz.apache.org/bugzilla/show_bug.cgi?id=61424 for details of how this may trigger a StackOverflowError Given
			 * these reported errors, catch Throwable to ensure any other edge cases are also caught */
