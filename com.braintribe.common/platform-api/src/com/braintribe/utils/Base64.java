...
public class Base64 {
	...
	protected static Logger logger = Logger.getLogger(Base64.class);
	...
	/**
	 * Encodes a String into Base64 notation. Does not GZip-compress data.
	 *
	 * @param source
	 *            The data to convert
	 * @since 1.4
	...
	public static String encodeString(final String source) {
		final byte[] sourceBytes = StringTools.isEmpty(source) ? new byte[0] : source.getBytes();
		return encodeBytes(sourceBytes, 0, sourceBytes.length, NO_OPTIONS);
	} // end encodeString
	 ...
	 * @since 2.0
	 */
	public static String encodeString(final String source, final int options) {
		final byte[] sourceBytes = StringTools.isEmpty(source) ? new byte[0] : source.getBytes();
		return encodeBytes(sourceBytes, 0, sourceBytes.length, options);
	} // end encodeString
	 ...
	 * @since 1.4
	 */
	public static String encodeString(final String source, final int off, final int len) {
		final byte[] sourceBytes = StringTools.isEmpty(source) ? new byte[0] : source.getBytes();
		return encodeBytes(sourceBytes, off, len, NO_OPTIONS);
	} // end encodeString
	 ...
	 * @since 2.0
	 */
	public static String encodeString(final String source, final int off, final int len, final int options) {
		final byte[] sourceBytes = StringTools.isEmpty(source) ? new byte[0] : source.getBytes();
		return encodeBytes(sourceBytes, off, len, options);
	} // end encodeString
	...
	/**
	 * Very low-level access to decoding ASCII characters in the form of a byte array to String. Does not support automatically gunzipping or any
	 ...
	 * @since 1.3
	 */
	public static String decodeToString(final byte[] source, final int off, final int len) {
		return new String(decode(source, off, len));
	} // end decodeToString

	/**
	 * Decodes data from Base64 notation to String, automatically detecting gzip-compressed data and decompressing it.
	 *
	 * @param s
	 *            the string to decode
	 * @return the decoded data
	...
	public static Object decodeToObject(final String encodedObject) {
		...
		try {
			...
			e.printStackTrace();
			// obj = null; can only be null at this point
		} // end catch
	...
	public static String encodeFromFile(final String filename) {
		try {
			return Base64.encodeFromFile(filename, true);
		} catch (final Exception e) {
			return null;
		}
	}
	 ...
	 * @since 2.1
	 */
	public static String encodeFromFileWithException(final String filename) throws Exception {
		return Base64.encodeFromFile(filename, false);
	}
	 ...
	 * @since 2.1
	 */
	private static String encodeFromFile(final String filename, final boolean useStandardStreamsOnly) throws Exception {
		...
		try {
			...
			if (useStandardStreamsOnly) {
				System.err.println("Error encoding from file " + filename);
			} else {
				throw new Exception(String.format("error while encoding file %s: %s", filename, e.getMessage()), e);
			}
		} finally {
			try {
				bis.close();
	...
	/**
	 * A {@link Base64.InputStream} will read data from another <tt>java.io.InputStream</tt>, given in the constructor, and encode/decode to/from
	...
	public static class InputStream extends java.io.FilterInputStream {
		...
		public InputStream(final Reader reader, int options) {
			this(new ReaderInputStream(reader), options);
		}
	...
	/**
	 * A {@link Base64.OutputStream} will write data to another <tt>java.io.OutputStream</tt>, given in the constructor, and encode/decode to/from
	 * Base64 notation on the fly.
	 *
	 * @see Base64
	 * @since 1.3
	...
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		public OutputStream(final Writer outWriter, final int options) {
			this(new WriterOutputStream(outWriter, "ASCII"), options);
		}
		...
		public void flushBase64() throws java.io.IOException {
			if (this.position > 0) {
				if (this.encode) {
					super.out.write(encode3to4(this.b4, this.buffer, this.position));
					this.position = 0;
				} // end if: encoding
				else {
