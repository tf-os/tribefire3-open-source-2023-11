...
function replace_echo_lines {
	...
	LIST_SH_FILES=$(find /opt/tribefire/host/bin -type f -name '*.sh')
	for SH_FILE in ${LIST_SH_FILES}; do
		# Replace echo->logJson.sh in lines that start with 'echo "' with 0 or more white spaces in the beginning of the line
		sed -i 's/^\([ ]*\)echo[ ]\+["]\(.*\)["]\(.*\)$/\1logJson.sh -m "\2"\3/g' "${SH_FILE}"
		# Replace echo->logJson.sh in lines that start with '*:*) echo' with 0 or more white spaces in the beginning of the line
		sed -i 's/^\([ ]*\)\([\*]:[\*][\)]\)\([ ]*\)echo[ ]\+["]\(.*\)["]\(.*\)$/\1\2\3logJson.sh -m "\4"\5/g' "${SH_FILE}"
		# Replace empty echo lines
		sed -i 's/^\([ ]*\)echo$/\1logJson.sh -m ""/g' "${SH_FILE}"
		# After several replacements (echo->logJson.sh), we need to check if there are still some lines that contain an 'echo' command.
...
if [ ! -z "${ENVIRONMENT_SPECIFIC_URL_REWRITE_RULES}" ]; then
	...
	URL_REWRITE_CONFIG=/opt/tribefire/host/conf/Catalina/localhost/rewrite.config
...
if { [ -n "${KUBERNETES_SERVICE_HOST}" ] && [ "${JSON_LOGGING_ENABLED}" != "false" ]; } || [ "${JSON_LOGGING_ENABLED}" == "true" ]; then
	logJson.sh -m "${ENVIRONMENT_VARS}"
	replace_echo_lines
	cp /opt/tribefire/host/conf/json-logging.properties /opt/tribefire/host/conf/logging.properties
else
	echo "${ENVIRONMENT_VARS}"
	cp /opt/tribefire/host/conf/default-logging.properties /opt/tribefire/host/conf/logging.properties
fi
rm -f /opt/tribefire/host/conf/default-logging.properties
rm -f /opt/tribefire/host/conf/json-logging.properties
rm -f /opt/tribefire/host/conf/ROOT_logging.properties

source /opt/tribefire/host/bin/catalina.sh ${OPTIONAL_CATALINA_ARGS} run
