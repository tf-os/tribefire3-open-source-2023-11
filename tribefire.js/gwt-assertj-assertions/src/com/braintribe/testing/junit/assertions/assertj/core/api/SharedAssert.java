...
/**
 * Provides assertions that are shared by custom assertions and custom assertion extensions. This is used as a
 * work-around, since Java doesn't support multiple inheritance for classes.
 *
...
public interface SharedAssert<S extends AbstractAssert<S, A>, A> extends Assert<S, A> {
	...
	/**
	 * Delegates to method {@link AbstractAssert#withFailMessage(String, Object...)} on the specified
	 * <code>assertInstance</code> passing the given <code>errorMessage</code> and <code>arguments</code>. The purpose
	 * of this method is to be able to call the delegate method from this interface. Otherwise there is no reason to use
	 * this method. Instead one should directly call the delegate.
	 */
	...
	/**
	 * Returns the actual value for the given <code>assertInstance</code>. The expected field name is
	 * {@value #ACTUAL_FIELD}.
	 */
	...
	/**
	 * Returns the {@link Logger} of the given <code>assertInstance</code>. The expected field name is
	 * {@value #LOGGER_FIELD}.
	 *
	...
	/**
	 * Returns a string representation of the passed <code>object</code>. This is e.g. used to distinguish between
	 * <code>null</code> and String <code>"null"</code> or between a number and the same number as string.
	 */
	...
	/**
	 * Performs an <code>equals</code> check similar to {@link #isEqualTo(Object)}, but with a more verbose message in
	...
	/**
	 * Performs an <code>equals</code> check similar to {@link #isEqualTo(Object)}, but with a more verbose message in
	 * case of an assertion error. This is similar to {@link #isEqualToWithVerboseErrorMessage(Object)}, but it also
	 * logs the info on {@link LogLevel#ERROR ERROR} level using the <code>Assert</code>'s
	 * {@link #logger(AbstractAssert) Logger}.
	 */
	...
	/**
	 * Returns a verbose error message, if the given objects are not equal (or both <code>null</code>). For example, if
	 * the not even the types are the same, their fully qualified names will be included in the message.
	 *
