...
/**
 * Provides assertions related to method invocations which are passed as lambda expressions. This can e.g. be used to
 * assert that executing method <code>doSomething</code> takes less than <code>3</code> seconds and fails with an
 ...
 * </pre>
 *
 * Please note that this class provides multiple methods that actually do the same, for example,
 * {@link #withException()} and {@link #throwingException()} or {@link #inLessThan(long, ChronoUnit)} and
 * {@link #afterLessThan(long, ChronoUnit)}. The simple reason is that depending on the combination and order of the
 * assertion methods certain method names just allow for creating more readable English sentences.
 *
 * <pre>
...
public class MethodInvocationAssert<T> {
	...
	/**
	 * Same as {@link java.util.concurrent.Callable}, but since we use our own type for
	 * {@link ExecutableWithoutReturnValue}, we also use our own type here to be consistent.
	 */
	@FunctionalInterface
	...
	/**
	 * Asserts that method execution takes less than the specified <code>minAmount</code> and more than less than the
	 * specified <code>maxAmount</code> of time.
	 */
	...
	/**
	 * Asserts that method execution throws a {@link Throwable}, which can be further checked via the returned
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert throwingThrowableWhich() {
		throwing(Throwable.class);
		return new ThrowableAssert(throwable);
	}

	/**
	 * Asserts that method execution throws an {@link Exception}, which can be further checked via the returned
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert throwingExceptionWhich() {
		throwingException();
		return new ThrowableAssert(throwable);
	}

	/**
	 * Asserts that method execution throws a {@link RuntimeException}, which can be further checked via the returned
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert throwingUncheckedExceptionWhich() {
		throwingUncheckedException();
		return new ThrowableAssert(throwable);
	}

	/**
	 * Asserts that method execution throws an {@link Error}, which can be further checked via the returned
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert throwingErrorWhich() {
		throwingUncheckedException();
		return new ThrowableAssert(throwable);
	}
	 ...
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert withThrowableWhich() {
	 ...
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert withExceptionWhich() {
	 ...
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert withUncheckedExceptionWhich() {
	 ...
	 * {@link ThrowableAssert}.
	 */
	public ThrowableAssert withErrorWhich() {
	...
	private void checkThrowable(Class<? extends Throwable> expectedSuperType) {
		if (!expectedSuperType.isInstance(throwable)) {
			fail("Invocation failed with " + throwable.getClass().getName() + ", although " + throwable.getClass().getName() + " was expected!\n"
	...
	/**
	 * Executes the method and afters {@link #checkStatus(Boolean) checks the status} (against
	...
	/**
	 * Used to fluently configure method invocation settings and then trigger the method execution via
	 * {@link #finishes()}, {@link #succeeds()} or{@link #fails()}.
	 *
	 * @author michael.lafite
	 *
	 * @param <U>
	...
	public static class MethodInvocationSettings<U> {
		...
		/**
		 * Specifies to execute the method sequentially. This is the default. This setting only makes sense when
		 * executing the method {@link #nTimes(int) more than once}. <br>
		 * Note that concurrent execution isn't supported yet, but this will be added in future versions of this class.
		 * Let us know if that would be useful for you!
		 */
