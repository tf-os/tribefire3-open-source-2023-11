...
/**
 * Provides custom {@link CharSequence} asserts used by {@link ExtendedCharSequenceAssert} and
 * {@link ExtendedStringAssert}. The interface is required, because {@link ExtendedStringAssert} otherwise would have to
 * extend two classes (one that holds the methods of this interface and {@link AbstractStringAssert}), which is not
 * possible in Java.
 *
...
public interface SharedCharSequenceAssert<S extends AbstractAssert<S, A>, A extends CharSequence> extends SharedAssert<S, A> {
	...
	/**
	 * Asserts that the number of occurrences of the <code>searchedSequence</code> is less than or equal
	 * <code>maxExpectedNumberOfOccurrences</code>.
	 */
	...
	/**
	 * Similar to {@link SharedAssert#isEqualToWithVerboseErrorMessage(Object)}, but in addition provides detailed
	 * information about the first difference found in the compared <code>CharSequence</code>s. This also includes the
	 * character codes (which can be helpful for invisible characters, e.g. different line separators).
	 *
	...
	/**
	 * This is similar to {@link #isEqualToWithVerboseErrorMessage(CharSequence)}, but it also logs the info on
	 * {@link LogLevel#ERROR ERROR} level using the <code>Assert</code>s {@link SharedAssert#logger(AbstractAssert)
	 * Logger}.
	 */
	@Override
	...
	/**
	 * Similar to {@link SharedAssert#createVerboseErrorMessageIfObjectsAreNotEqual(Object, Object)}, but adds detailed
