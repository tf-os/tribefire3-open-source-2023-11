...
/**
 * TODO: proceeding must check the type invariance for the ServiceRequest in order to keep the selected ServiceProcessor a valid match
 * @author Dirk Scheffler
 *
 */
@SuppressWarnings("deprecation")
public class ServiceProcessingChain implements ServiceProcessor<ServiceRequest, Object>, ProceedContext {
	private static final ServiceProcessor<ServiceRequest, Object> DEFAULT_PROCESSOR = (c, r) -> { throw new UnsupportedOperationException("No service processor mapped for request: " + r); };
	...
	private static class InterceptingServiceProcessorBuilderImpl extends AbstractServiceInterceptionBuilder implements InterceptingServiceProcessorBuilder {
	...
	private class ImmutableProceedContext implements ProceedContext, ServiceProcessor<ServiceRequest, Object> {
		...
		public <T> T proceed(ServiceRequest serviceRequest) {
			if (index < aroundProcessors.size()) {
				return (T)aroundProcessors.get(index).process(requestContext, serviceRequest, new ImmutableProceedContext(requestContext, index + 1));
			}
			else {
