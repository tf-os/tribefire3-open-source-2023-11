...
/**
 * The <code>Environment</code> can be used to run integration tests (that depend on external components, e.g. database,
 * external system like Documentum, files, etc.) in multiple environments (e.g. with different database connection
 * settings). Basically it's just a convenience class that holds environment settings that are read from environment
 * specific {@link Properties} files (and a {@link #DEFAULTENVIRONMENT_ENVIRONMENTID default} properties file as
 * fallback).
 * <p>
 * Properties files must be placed in {@link #getEnvironmentFolder() environment folder}, by default
 * {@value #ENVIRONMENTSETTINGS_FOLDER_DEFAULT}. Properties files are named
 * <code>[PREFIX][ENVIRONMEN_ID].properties</code>. The prefix can be {@link #setEnvironmentFileNamePrefix(String)
