 ...
 * <p>
 * Simple helper to {@link #build() build} a {@link Path} providing configuration settings such as whether to actually
 * {@link #createOnFileSystem() create the path on the file system} or whether to {@link #deleteOnExit() delete it on
 * exit}. Settings are configurable via fluent API. Entry point is {@link #withSettings()}. Example:
 *
 * <pre>
...
public interface PathBuilder {
	...
	public static final PathType DEFAULT_PATHTYPE = PathType.Any;
	public static final PathExistence DEFAULT_EXISTSCHECK = PathExistence.MayExist;
	public static final CreateParents DEFAULT_PARENTSCREATION = CreateParents.OnCreate;
	public static final AddTimestampSuffix DEFAULT_ADDTIMESTAMPSUFFIX = AddTimestampSuffix.Never;
	public static final boolean DEFAULT_CREATEONFILESYSTEM = false;
	public static final boolean DEFAULT_DELETEONEXIT = false;
	...
	public enum CreateParents {
		...
		Always,
		/**
		 * Create parent directories, if they don't exist and if {@link PathBuilder#createOnFileSystem() the path is
		 * created on file system}.
		 */
		OnCreate,
		/**
		 * Never create parent directories. This causes a failure, if {@link PathBuilder#createOnFileSystem() the path
		 * is supposed to be created on file system} and the parents don't exist.
		 */
		Never;
	}
	...
	public enum AddTimestampSuffix {
		...
		IfRequired,
		/**
		 * Never add timestamp suffix. If the path {@link PathExistence#MustNotExist must not exist}, this will cause a
		 * failure.
		 */
		Never;
	}
	...
	/**
	 * Sets the base dir. This can be combined with a {@link #relativePath relative} {@link #path path}.
	 * Default is the {@link #baseDirIsWorkingDir() working directory}.
	 */
	...
	/**
	 * Sets the {@link #createOnFileSystem() created} path to be deleted on exit. Default is
	 * {@value #DEFAULT_DELETEONEXIT}.
	 */
	PathBuilder deleteOnExit();

	/**
	 * Specifies whether the {@link #createOnFileSystem() created} path shall be deleted on exit. Default is
	 * {@value #DEFAULT_DELETEONEXIT}.
	 */
	...
	/**
	 * Sets the path which may be relative to the {@link #baseDir(String) base dir} or absolute. If it is absolute, the
	 * base dir will be ignored. Specifying the path is mandatory.
	 */
	PathBuilder path(String path, String... more);

	/**
	 * Same as {@link #path}, but only accepts relative paths (relative to the {@link #baseDir(String) base
	 * dir}).
	 *
	...
	/**
	 * Specifies whether the {@link #path path} is expected to already exist (on the file system). Default is
	 * {@value #DEFAULT_EXISTSCHECK}.
	 */
	...
	/**
	 * Delegates to {@link #createParents(CreateParents)} passing {@link CreateParents#Always}, if
	 * <code>createParents</code> is <code>true</code>, otherwise {@link CreateParents#Never}.
	 */
	...
	/**
	 * Delegates to {@link #addTimestampSuffix(AddTimestampSuffix)} passing {@link AddTimestampSuffix#Always}, if
	 * <code>addTimestampSuffix</code> is <code>true</code>, otherwise {@link AddTimestampSuffix#Never}.
	 */
	...
	/**
	 * Builds the path and returns it as a {@link File} instance. This first delegates to {@link #build()} and therefore
	 * throws same exceptions.
	 */
	File buildFile();

	/**
	 * Creates a new {@link PathBuilder} instance ready to be configured. This can be used as entry point for the fluent
	 * configuration, e.g.
	...
	static class PathBuilderImpl implements PathBuilder {
		...
		public Path build() {
			...
			if (!parentExists) {
				// TODO: for all parent dirs (below base dir) it'd probably make sense to also set delete on exit (if enabled)
			...
			while (true) {
				...
				if (resultExists && pathExistence.equals(PathExistence.MustNotExist)) {
					if (addTimestampSuffix.equals(AddTimestampSuffix.Never))
						throw new RuntimeException("File already exists:" + result.toFile());
				...
				if (!resultExists && pathExistence.equals(PathExistence.MustExist)) {
					throw new RuntimeException("File not found:" +  resultAsFile);
				}
					...
					if (!success) {
						if (!timestampAdded) {
							...
							if (exception == null) {
								...
								throw new RuntimeException("Couldn't create " + result + "!", exception);
							}
						}
						tries++;
						if (tries > 1000) {
							throw new RuntimeException("Couldn't create " + result + ", even after trying multiple time stamp based names!",
									exception);
						}
						continue;
					}
