...
/**
 * Convenience class that may be used as a super type when writing unit tests. It e.g. provides methods to easily access
 ...
 * <p>
 * The class also provides convenience methods to create temporary files or folders (in temporary files directory). The
 * names are based on simple test class names. If the path already exists, a timestamp suffix is added. Examples:
 *
 * <pre>
 * newTempDir();
 * newTempFile();
 ...
 * 
 * <p>
 * In its static initializer this class automatically
 * {@link JulConfigurationHelper#setSimpleConsoleOnlyLoggingConfigurationUnlessAlreadyConfigured() configures JUL}, so
 * that one doesn't get log messages in two-line format written to <code>System.err</code> (which is the default). Note
 * that logging configuration settings may be changed in future releases, thus one shouldn't expect the logging output
 * to look exactly as it currently does.
 * <p>
 * For logging, sub classes can just use the <code>protected</code> {@link HasSharedLogger#logger logger} field or
 * declare their own {@link Logger} in a field named {@value HasSharedLogger#LOGGER_FIELD}. The own logger is only
 * needed for logging from <code>static</code> methods. For more information see {@link HasSharedLogger}.
 *
...
public class AbstractTest extends HasSharedLogger {
	...
	/**
	 * Returns the test resources directory. Other methods from this class, e.g. {@link #testPath(String)} or
	 * {@link #existingTestFile(Class, String)} get the base resources directory by calling this method.<br>
	 * When this method is invoked the first time (and {@link #resDirPath} is not set yet), it checks if one of the
	 * following paths exist:
	 * <ol>
	 * <li><code>[working directory]/src/test/resources</code>: This is the expected directory for test resources in
	 * artifacts with a standard Maven structure.</li>
	 * <li><code>working directory]/res</code>: This an alternative, which is e.g. used for separate test artifacts with
	 * Ant-based building.</li>
	 * </ol>
	 *
	 * If one wants to use another folder, one has to programmatically set <code>static</code> field {@link #resDirPath}
	...
	/**
	 * Gets the test directory path for the specified <code>testClass</code>. Same as {@link #testPath(Class, String)}
	...
	/**
	 * Gets the path for a file/directory (specified as <code>relativePath</code>) inside the test
	 * {@link #newTestPathBuilder(Class) directory} of the specified <code>testClass</code>.
	 * <p>
	 * Example: for class <code>com.braintribe.ExampleTest</code> and relative path <code>path/to/testfile.txt</code>
	 * the method returns <code>[TEST_RESOURCES_DIRECTORY]/com/braintribe/ExampleTest/path/to/testfile.txt</code>.
	 *
	 * @see #resDirPath
	 *
	 * @see #testPath(String)
	...
	/**
	 * {@link #testFile(Class, String, PathExistence) Gets the test file path} for the specified
	 * <code>relativePath</code> and returns it as <code>File</code>.
	 */
	...
	/**
	 * Gets the {@link #testPath(Class, String) test path} for the specified <code>relativePath</code> and returns it as
	 * <code>File</code>.
	 *
	 * @throws AssertionException
	 *             if the path exists, but is not a {@link PathType#File file} or if <code>expectedExistence</code>
	 *             check fails.
	 *
	...
	/**
	 * {@link #testFile(Class, String, PathExistence) Gets the test directory path} for the specified
	 * <code>relativePath</code> and returns it as <code>File</code>.
	 */
	...
	/**
	 * Gets the {@link #testPath(String) test path} for the specified <code>relativePath</code> and returns it as
	 * <code>File</code>.
	 *
	 * @throws AssertionException
	...
	/**
	 * Returns a {@link PathBuilder} with the {@link PathBuilder#baseDir(String) base dir} set to the test directory for
	 * given <code>testClass</code>. This test directory is a path relative to to {@link #resDirPath() resources
	 * directory} and based on the fully qualified name of the <code>testClass</code>.<br>
	 * The easiest way to explain this is with an example. For resources directory <code>./res</code> and class
	 * <code>com.acme.ExampleTest</code> possible test directories are:
	 * <ul>
	 ...
	 * <p>
	 * Using this approach each test class gets its own directory for test resources. This makes it possible to easily
	 * organize test resources.
	 * <p>
	 * Note that for temporary files one can also use the methods provided by {@link TestTools}, e.g.
	 * {@link TestTools#newTempFileForClass(Class)}.
	 *
	 * @throws AssertionException
	...
	/**
	 * Creates a new directory in temporary files directory. Same as {@link #newTempDir(String)} with a relative path
	 * based on simple test class name.
	 */
	...
	/**
	 * Creates a new directory in temporary files directory based on the given <code>relativeFilePath</code>. If
	 * required, a timestamp suffix is added to make sure the directory doesn't exist yet. The directory will be
	 * {@link File#deleteOnExit() deleted on exit}, if it is empty.
	 */
	public File newTempDir(String relativeFilePath) {
		return newTempFileOrDir(relativeFilePath, PathType.Directory);
	}

	/**
	 * Creates a new file in temporary files directory. Same as {@link #newTempFile(String)} with a relative path based
	 * on simple test class name.
	 */
	...
	/**
	 * Creates a new file in temporary files directory based on the given <code>relativeFilePath</code>. If required, a
	 * timestamp suffix is added to make sure the file doesn't exist yet. The file will be {@link File#deleteOnExit()
	 * deleted on exit}.
	 */
	public File newTempFile(String relativePath) {
		return newTempFileOrDir(relativePath, PathType.File);
	}

	private File newTempFileOrDir(String relativePath, PathType pathType) {
