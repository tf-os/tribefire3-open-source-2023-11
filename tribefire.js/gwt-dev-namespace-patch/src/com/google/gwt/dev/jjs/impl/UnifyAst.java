...
import com.braintribe.gwt.genericmodel.build.GwtCompilationInfo;
...
public class UnifyAst {
	...
	public static boolean MINIMIZE_BUILD = minimizeBuild();
	
	private static boolean minimizeBuild() {
		String value = System.getProperty("minimize");
...
/**
  ...
  private class UnifyVisitor extends JModVisitor {
    ...
    /**
     * PGA: This method was added to collect the class literal information.
     */
    @Override
	public boolean visit(JClassLiteral x, Context ctx) {
    	if (MINIMIZE_BUILD)
    		GwtCompilationInfo.addEssentialType(x.getRefType().getName());
    	
			// In case troubleshooting is needed, this is helpful (to see how we reached a certain type):

//			if (AbsenceInformation.class.getName().equals(x.getRefType().getName())) {
//				System.out.println(x.getRefType().getShortName() + ": ");
//				bt_printPrecursorPath(currentMethod);
//			}
   ...
   */
  private final Set<String> processedStaleTypeNames = Sets.newHashSet();
  ...
  private final boolean incrementalCompile;
  ...
  private void flowInto(JMethod method) {
    ...
    precursors.put(method, mainLoopMethod);
...
private void resolveSpecialization(JMethod method) {
  ...
  private void resolveType(JDeclaredType type) {
    ...
    for (JInterfaceType intf : type.getImplements()) {
      resolvedInterfaces.add(translate(intf));
    }
  ...
  private <T extends JType> List<T> translate(List<T> types) {
	...
	// ###################################################
	// ## . . . . . . Patched/Extra methods . . . . . . ##
	// ###################################################

	JMethod mainLoopMethod;
	Map<JMethod, JMethod> precursors = new HashMap<>();

	/**
	 * Main loop: run through the queue doing deferred resolution. We could have made this entirely recursive, but a
	 * work queue uses much less max stack.
	 */
	...
	private void mainLoopMinimal() {
		System.out.println("THIS BUILD IS MINIMIZING THE NUMBER OF ENTITIES");
		
		UnifyVisitor visitor = new UnifyVisitor();
		/* PGA: We visit the GwtGmPlatform.getTypeReflection() method only when all other possible methods were visited.
		 * This method triggers the generator, and we want to collect all the relevant class literals first. */
		while (!methodsPending.isEmpty()) {
			mainLoopMethod = methodsPending.poll();

			if (methodsPending.isEmpty() || !shouldBeDelayed(mainLoopMethod)) {
				visitor.accept(mainLoopMethod);
			} else
				methodsPending.offer(mainLoopMethod);
		}
	}

	private static boolean shouldBeDelayed(JMethod method) {
		String signature = method.toString();
		return is_GwtGmPlatform_getTypeReflection(signature) || is_GwtGenericModelTypeReflection_initialTypes(signature) || is_initialTypes(signature);
	}

	private static String s = "com.braintribe.gwt.genericmodel.client.GwtGmPlatform.getTypeReflection()Lcom/braintribe/model/generic/reflection/GenericModelTypeReflection;";

	private static boolean is_GwtGmPlatform_getTypeReflection(String signature) {
		return signature.equals(s);
	}

	private static boolean is_GwtGenericModelTypeReflection_initialTypes(String signature) {
		return signature.equals("com.braintribe.gwt.genericmodel.client.reflection.GwtGenericModelTypeReflection.nextTypes()Ljava/util/stream/Stream;");
	}
	
	private static boolean is_initialTypes(String signature) {
		return signature.startsWith("com.braintribe.gwt.genericmodel.client.itw.InitialEntityTypesImpl")
				&& signature.endsWith("nextTypes()Ljava/util/stream/Stream;");
	}

	protected void bt_printPrecursorPath(JMethod method) {
		System.out.println("PRECURSORS:");
		do {
			System.out.println("-> " + method);
			method = precursors.get(method);
		} while (method != null);
	}
