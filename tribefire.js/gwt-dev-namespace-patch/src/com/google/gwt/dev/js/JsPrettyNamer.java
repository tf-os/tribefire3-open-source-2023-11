...
public class JsPrettyNamer extends JsNamer {
  ...
  private Set<String> changeNames(JsScope scope) {
    ...
    for (JsName name : scope.getAllNames()) {
      ...
      rename(name, scope, taken, suffixCounters, ensureFirstCharNotLower);
  ...
  private void rename(JsName name, JsScope scope, Set<String> taken,
      HashMap<String, Integer> suffixCounters, boolean ensureFirstCharNotLower) {
    ...
    //gwt 2.8.0 has a problem when using generic along with method reference. Fixing it here.
    prefix = NamerTools.fixGenericIssue(prefix);
    
    if (ensureFirstCharNotLower) {
    	prefix = NamerTools.ensureFirstCharIsNotValidForPropertyNames(prefix);
    }
    ...
    } else if (ensureFirstCharNotLower) {
        name.setShortIdent(prefix);
    }
