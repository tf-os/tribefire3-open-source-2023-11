...
/**
 * Implementation of {@link CustomEqualsCollection} that uses a configurable {@link #getDelegate() delegate collection}
 * but intercepts all methods where the {@link #getEqualsCheck() equals check} is relevant (i.e.
 * <code>add, remove, contains</code>, etc.). Note that this implementation may be VERY slow when compared to the
 * delegate collection, because it usually has to iterate through the elements (instead of using hash codes, etc.).
 *
 * @see CustomEqualsSet
