...
public final class MathTools {
	 ...
	 * @return the minimum number or <code>null</code> if no valid number is found (and also if no numbers are specified
	 *         at all).
	 */
	...
	public static BigDecimal getBigDecimalValue(final String valueString, final BigDecimal defaultValueForEmptyNumberString,
		...
		try {
			...
			if (replaceCommasWithDots) {
				normalizedValueString = normalizedValueString.replaceAll(",", ".");
			}
	...
	/**
	 * Normalizes the passed <code>decimalString</code>. The method converts <code>,</code> to <code>.</code> and then
	 * removes all but the last <code>.</code> (since that should be the decimal separator). Furthermore the string is
	 * trimmed.
	 */
	...
	/**
	 * Returns the passed <code>value</code> unless it's less than the <code>minimumValue</code> or greater than the
	 * specified <code>maximumValue</code> (which both may be <code>null</code>), in which case the minimum/maximum is
	 * returned.
	 */
	...
	/**
	 * Convenience method to convert a long value to a Double number and divided it by 1 billion. This is useful if you
	 * have a number (e.g., disk space, cpu speed, network speed, etc) and want to have the same number in GB, Ghz, ...
	 *
	 * @param bytes
	 *            The actual value.
	 * @param si
	...
	/**
	 * Applies a minimum and maximum to a given number. Hence, the provided
	 * value must be within the range of min and max.
	 * This is basically the same as using {@link Math#min(int, int)} and {@link Math#max(int, int)}
	 * on the same input value.
	 * 
	 * @param value The value that should be clipped.
	 * @param min The minimum allowed value.
	 * @param max The maximum allowed value.
	 * @throws IllegalArgumentException Thrown if the maximum is smaller than the minimum
	 ...
	 * Applies a minimum and maximum to a given number. Hence, the provided
	 * value must be within the range of min and max.
	 * This is basically the same as using {@link Math#min(long, long)} and {@link Math#max(long, long)}
	 * on the same input value.
	 * 
	 * @param value The value that should be clipped.
	 * @param min The minimum allowed value.
	...
	/**
	 * Computes the provided number to the power as provided in the second parameter.
	 * This is a convenience method as the standard {@link Math#pow(double, double)} method
	 * only accepts double parameters.
	 * 
	 * @param number The number that should be operated on.
