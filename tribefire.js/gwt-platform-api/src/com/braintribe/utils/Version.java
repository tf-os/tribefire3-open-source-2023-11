 ...
 *
 * There is no limit on how many levels the version may have. Each part of the version must be a number (i.e., an int
 * value). No Strings are allowed. This class is able to parse Version Strings as well as comparing two Version objects
 * for equality or rank (i.e., which version is higher).
 *
 */
public class Version implements Comparator<Version> {
	...
	/**
	 * Creates a Version by copying the version information from the provided template. If the provided Version is, for
	 * example, 1.1.23, this newly created Version will contain the exact same version information.
	 *
	 * @param other
	...
	/**
	 * Creates a new Version object based on the individual parts of the version. The first element of the array would
	 * be the major version, the second the minor version, and so on. THere is no limit on the number of parts.
	 *
	 * @param versionParts
	 *            The numbers for the individual parts (levels) of the version. If an empty array is provided, the
