...
public class OsTools {
	...
	/**
	 * Returns the operating system. Note that currently the result is based on a simple check, if the operating system
	 * name (based on property <code>os.name</code>) contains (ignoring case) "mac" (or "darwin"), "windows" or "linux".
	 * Otherwise the method returns {@link OperatingSystem#Other}. Please note that this may be improved in future
	 * versions. For example, further OS types may be added, which means one must not rely on <code>Other</code> to be
	 * returned.
	 */
	...
	/**
	 * Returns <code>true</code>, if the JVM is running on a Windows {@link #getOperatingSystem() operating system},
	...
	/**
	 * Returns the architecture bits based property <code>os.arch</code>. Note that currently there is only a simple
	 * check, if the architecture name contains "64", in which case <code>64</code> is returned, otherwise
	 * <code>32</code>. This may be changed/improved in the future. Therefore one must not rely on this!
	 */
	...
	/**
	 * Simple helper class used to build file names that depend on the {@link OsTools#getOperatingSystem() operating
	 * system}, {@link OsTools#getArchitetureBits() architecture bits}, etc.
	 *
	 * @author michael.lafite
	 */
