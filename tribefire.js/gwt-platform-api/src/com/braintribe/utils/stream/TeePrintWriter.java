...
/**
 * This class is a wrapper around a provided PrintWriter. All actions are invoked with the
 * delegate PrintWriter. 
 * The functionality of this wrapper is that it additionally writes the content to a secondary ("tee") writer.
 * Output to the secondary Writer can be stopped irrevocably by invocing {@link #stopTee()}.
 * This implementation will first try to write the content to the tee Writer and then
 * to the main delegate PrintWriter. If the tee Writer throws an exception, the data will not be written to the 
 * delegate PrintWriter. If the delegate PrintWriter throws an Exception, the tee Writer has already gotten the information.
 */
