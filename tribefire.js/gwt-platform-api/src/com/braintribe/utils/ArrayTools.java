 ...
 * {@link com.braintribe.utils.lcd.ArrayTools}.
 *
...
public final class ArrayTools extends com.braintribe.utils.lcd.ArrayTools {
	...
	/**
	 * Converts a list of lists to a 2-dimensional array. Each row in the returned array has the same (i.e. maximum)
	 * length even if the lists are not the same size.
	 *
	 * @param listOfLists
	...
	/**
	 * Converts a list of lists of strings to a 2-dimensional array of strings. Each row in the returned array has the
	 * same (i.e. maximum) length even if the lists are not the same size. (There is also a method to create an array of
	 * objects (see {@link #createTwoDimensionalArrayFromListofLists(List)}, but no generic method yet.)
	 *
	 * @param listOfLists
	...
	/**
	 * Similar to {@link #merge(Class, Object[], Object...)}, but the component type of the resulting array is taken as
	 * the component type of the first parameter, which must however be the exact same as component type of second
	 * parameter (otherwise {@link ArrayStoreException} may be thrown).
	 */
	...
	/**
	 * Creates a new array of {@code componentType}, which is a simple concatenation of arrays provided as parameters in
	 * respective order. Note that this may be a little tricky due to a mixture of generics a arrays, so one can for
	 * example do something like: {@code merge(Collection.class, arrayofLists, arrayOfSets)}.
	 */
	...
	/**
	 * merges all kind of arrays (also primitive ones) by appending them into one array that exactly has the required
	 * length which is the sum of the lengths of the given arrays
	 */
	...
	/**
	 * Returns an array containing all the elements of the passed collection. For more info see
	 * {@link #toArray(Collection, Object[])}.
	 */
	 ...
	 * 
	 * @param sourceArray The array that should be split.
	 * @param chunkSize The maximum size
	 * @return An array of byte arrays. If the sourceArray is null, the method also returns null.
	 * @throws IllegalArgumentException Thrown when the chunkSize is 0 or less.
	 */
	public static byte[][] splitArray(byte[] sourceArray, int chunkSize) {
	    ...
	    int rest = sourceArray.length % chunkSize;  // if rest>0 then our last array will have less elements than the others 
	    // then we check in how many arrays we can split our input array
	    int chunks = sourceArray.length / chunkSize + (rest > 0 ? 1 : 0); // we may have to add an additional array for the 'rest'
