...
/**
 * <p>
 * A helper class that's used to get string representations of objects where implementing the {@link Object#toString()}
 * method in their respective classes is not feasible. The <code>CustomToStringBuilder</code> is useful especially when
 * working with container objects (i.e. collections, maps, arrays), because it relies on its own
 * <code>toString(Object)</code> method to print the contents of these container classes. Furthermore it sorts maps and
 * sets, shows the contents of arrays (instead of just the arry object), etc.
 * </p>
 * <p>
 * Support for custom string representations can easily be added by passing a {@link CustomStringRepresentationProvider}
 * to {@link #CustomToStringBuilder(CustomStringRepresentationProvider)}. For objects for which the provider returns no
 * result (i.e. <code>null</code>), the {@link Object#toString()} default string representation will be returned.<br/>
 * Note that one can easily further customize the string representations by overriding any of the <code>protected</code>
 * methods of this class, e.g. {@link #nullToString()}, {@link #getCollectionElementSeparator()} ,
 * {@link #getMapEntryKeyValueSeparator()}, etc. For example, arrays are just converted to lists by default. This can be
 * changed by overriding {@link #arrayToStringAfterNullCheck(Object)}.
 * </p>
 *
...
public class CustomToStringBuilder {
	...
	protected <K, V> String mapToStringAfterNullCheck(final Map<K, V> map) {
		...
		final SortedMap<String, V> sortedMap = new TreeMap<String, V>();
	...
	/**
	 * A simple interface to get custom string representations for one or more types, i.e. representations different
	 * than the ones returned by {@link Object#toString()}.
	 *
	 * @author michael.lafite
	 */
	public interface CustomStringRepresentationProvider {
		 ...
		 * @return either the custom string representation or <code>null</code>, if the type of the passed
		 *         <code>object</code> is not supported.
		 */
