...
public class MapTools {
	...
	/**
	 * Converts the passed map to a map with <code>Integer</code> keys. The keys are parsed using
	...
	public static Map<Integer, String> convertMapToMapWithIntegerKeys(final Map<String, String> map) {
		final Map<Integer, String> integerKeysMap = new HashMap<Integer, String>();
	...
	/**
	 * Converts the passed map to a map with keys and values of type <code>String</code>. Keys and values are converted
	 * to <code>String</code>s using {@link CommonTools#toStringOrNull(Object)}.
	 *
	 * @param map
	 *            the map to convert.
	 * @return the converted map.
	 ...
	 *            Pairs of old values and new values (old values will be searched in the map and then replaced with new
	 *            values).
	 */
	...
	/**
	 * This method checks for each entry of <code>mapToAdd</code> if the passed <code>map</code> already contains a an
	 * entry with the same key and only adds the entry if it does not.
	 *
	 * @param map
	...
	/**
	 * Returns a new map that only contains the mappings of the passed <code>map</code> where the keys match the
	...
	/**
	 * Returns a map containing all key/value pairs of <code>mapToCheck</code> where the key is contained in
	 * <code>mapToCheck</code> but missing in <code>mapToCompareTo</code>.
	 */
	...
	/**
	 * Returns a map containing all key/value pairs of <code>mapToCompareTo</code> where the key is missing in
	 * <code>mapToCheck</code> but contained in <code>mapToCompareTo</code>.
	 */
	...
	/**
	 * Returns all key/value pairs from <code>mapToCheck</code> where the key exists in <code>mapToCompareTo</code> but
	...
	public static <K, V> Map<K, V> getSortedMap(final Map<K, V> map) {
		final SortedMap<K, V> sortedMap = new TreeMap<K, V>();
	...
	/**
	 * Converts the passed <code>map</code> to a string array where each element has the format [entry key] +
	 * <code>keyValueSeparatory</code> + [entry value].
	 */
	...
	/**
	 * Adds the <code>entriesToAdd</code> to the <code>map</code>. <code>entriesToAdd</code> may be <code>null</code>
	 * (in which case no entries are added).
	 *
	...
	/**
	 * Puts the passed <code>value</code> into the passed <code>map</code> unless the specified <code>key</code> is
	 * <code>null</code>.
	 */
	...
	/**
	 * Adds a new entry with the specified <code>key</code> to the passed <code>map</code> unless the String
	 * <code>value</code> is {@link StringTools#isEmpty(String) empty}.
	 */
	...
	/**
	 * Adds a new entry with the specified <code>key</code> to the passed <code>map</code> unless the <code>value</code>
	 * is <code>null</code>.
	 */
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if the map contains keys and/or values that are <code>null</code>, but no <code>nullString</code> is
	 *             set.
	...
	public static <K, V> Map<V, K> getInverseMap(final Map<K, V> map) {
		final Map<V, K> inverseMap = new HashMap<V, K>();
	...
	/**
	 * <p>
	 * Creates the intersection of {@link Map}s - a new {@link HashMap} only containing those entries that are contained
	 * in all of the source maps.
	 * </p>
	 * <p>
	 * <b>Note:</b> This method only checks the keys in the maps, the values in the resulting intersection-map will
	 * always be those of the first map.
	 * </p>
	 *
	 * @param maps
	 *            the source maps.
	 * @return A new {@link HashMap} containing the intersection of the maps <b>or</b> <code>null</code> if no maps were
	 *         passed to this method or any of them is null <b>or</b> the first map, if only one map has been passed to
	 *         this method.
	 */
	public static <K, V> Map<K, V> createIntersection(final Map<K, V>... maps) {
		...
		final Map<K, V> intersection = new HashMap<K, V>();
	...
	/**
	 * {@link GenericTaskWithContext#perform(Object) Performs} the specified <code>task</code> on the entries of the
	 * passed <code>map</code>. See {@link #withEntriesDo(Set, GenericTaskWithContext)}.
	 */
	...
	/**
	 * {@link GenericTaskWithContext#perform(Object) Performs} the specified <code>task</code> on the passed
	 * <code>entries</code>.
	 */
	...
	public static Map<Object, Object> getMap(final Object... keyAndValuePairs) {
		return putAllToMap(new HashMap<Object, Object>(), keyAndValuePairs);
	}
	...
	/**
	 * Creates a new map with the specified <code>keys</code> and <code>values</code>. The passed collections must be
	 * the same size (and not <code>null</code>).
	 *
	 * @param duplicateKeysAllowed
	 *            if enabled, duplicate keys are ignored (i.e. last value wins). If disabled, an
	...
	/**
	 * Creates a new map with the specified <code>keys</code> and lists of <code>values</code>. The passed collections
	 * must be the same size (and not <code>null</code>). Duplicate keys are expected, which is why the values of the
	 * map are lists.
	 *
	 * @throws IllegalArgumentException
	...
	/**
	 * Adds the passed <code>keysAndValues</code> to the <code>map</code>. Note that the method cannot make sure that
	 * keys are of type <code>K</code> and values of type <code>V</code>!
	 *
	 * @param map
	 ...
	 * @throws IllegalArgumentException
	 *             if the passed <code>map</code> is <code>null</code> or if their is a key without a value in
	...
	/**
	 * Converts the passed <code>map</code> to an array that contains pairs of keys and values, e.g. "key1", "value1, "
	 * key2", "value2, ...
	 *
	 * @param map
	 *            the map containing the keys and values
	 * @param componentType
	 *            the component type of the array. This parameter is used mainly to avoid casts in the calling method.
	...
	/**
	 * Turns {@code Map<K, V>} to {@code Map<K, W>} by applying the <tt>valueMapping</tt> on each value. The returned
	 * map preserves the order of iteration of the original map.
	 */
