 ...
 * <p>
 * Newly created and closed wrapper is in an "uninitialized" state, and the first invocation of the {@link #get()} method initializes it. Any
 * subsequent {@linkplain #get()} invocation always returns the same instance. If we close the wrapper, the next {@linkplain #get()} invocation
...
public class LazyInitialized<T> implements Supplier<T> {
	...
	/**
	 * Closes the underlying instance if possible. If a "destructor" was given via {@link #LazyInitialized(Supplier, Consumer) constructor}, it will
	 * be invoked with the underlying instance, otherwise if the instance is {@link AutoCloseable}, its <code>close</code> method will be called.
	 * <p>
	...
	private synchronized void syncClose() {
		...
		// This should only be nulled iff we have successfully closed our object, i.e. it's done as the last thing, after close was done
		itBox = null;
	}
