...
public class CollectionTools {
	...
	/**
	 * Removes duplicates from the passed <code>collection</code> (only the first element will be kept). The duplicates
	 * check supports <code>null</code> elements and (for non-<code>null</code> elements) relies on
	 * {@link Object#equals(Object) equals} check to find duplicates.
	 *
	 * @param collection
	 ...
	 * {@link #getStringRepresentation(Collection, String, String, String, String, boolean)} with prefix "[", suffix
	 * "]", separator ",", sizeIncluded==true.
	 */
	...
	/**
	 * Gets a string representation of the passed collection. The returned string has the following format
	 * "[collectionName][elementsPrefix][size] elements: [element_1 string][separator]...[separator][element_n
	 * string][elementsSuffix]" or "null" if a null pointer is passed. Method
	 * {@link StringTools#getStringRepresentation(Object)} is used to get the string representation for the individual
	 * elements.
	 */
	...
	/**
	 * Gets a string representation of the passed enumeration. The returned string has the following format
	 * "[enumerationName][elementsPrefix][size] elements: [element_1 string][separator]...[separator][element_n
	 * string][elementsSuffix]" or "null" if a null pointer is passed. Method
	 * {@link StringTools#getStringRepresentation(Object)} is used to get the string representation for the individual
	 * elements.
	 */
	...
	/**
	 * Returns a list of all elements in the passed collection. If a collection element is a {@link Collection} itself,
	 * then instead of adding the collection object, the elements of that collection are added to the list (using
	 * recursion).
	 */
	...
	/**
	 * Returns a new list containing all the elements of the passed <code>list</code> (which must not be
	 * <code>null</code>).
	 */
	...
	/**
	 * Returns all possible pairs of elements where one element is an element from the first collection and the other
	 * one an element from the second collection, i.e. the
	 * <a href="http://en.wikipedia.org/wiki/Cartesian_product">cartesian product</a> of both collections.
	 */
	public static <T, U> List<Pair<T, U>> createCartesianProduct(final Collection<T> collection1, final Collection<U> collection2) {
		...
		for (final T object1 : collection1) {
			for (final U object2 : collection2) {
				result.add(new Pair<T, U>(object1, object2)); // SuppressPMDWarnings (instantiation inside loop is fine
																// here)
			}
		}
		return result;
	...
	/**
	 * Returns the union of both collections, i.e. the elements that are contained in <code>collection1</code>,
	 * <code>collection2</code> or in both collections.
	 */
	...
	/**
	 * Gets the relative complement of <code>collection1</code> with respect to <code>collection2</code>, i.e. all
	 * elements that are contained in <code>collection2</code> but not in <code>collection1</code>.
	 */
	...
	/**
	 * Returns all elements that <code>collectionToCheck</code> contains in addition to the elements in
	 * <code>collectionToCompareTo</code>, i.e. all elements of <code>collectionToCheck</code> that are not contained in
	 * <code>collectionToCompareTo</code>.
	 */
	...
	/**
	 * Returns all elements that are missing in <code>collectionToCheck</code> but contained in
	 * <code>collectionToCompareTo</code>.
	 */
	...
	/**
	 * Adds the <code>elementsToAdd</code> to the <code>collection</code>. <code>elementsToAdd</code> may be
	 * <code>null</code> (in which case no elements are added).
	 *
	 ...
	 *            if <code>true</code> an exception is thrown if an empty string is found. If this option is enabled,
	 *            <code>removeEmptyStrings</code> must be disabled.
	 * @param trimStrings
	 ...
	 * @throws IllegalArgumentException
	 *             if <code>removeEmptyStrings</code> and <code>emptyStringsAreFatal</code> are both enabled or if
	 *             <code>emptyStringsAreFatal</code> is enabled and an empty string is found.
	 */
	...
	/**
	 * Adds the <code>element</code> to the <code>collection</code>, unless the <code>element</code> is
	 * <code>null</code>.
	 *
	 * @throws NullPointerException
	 *             if the <code>collection</code> is <code>null</code>.
	...
	/**
	 * Adds the <code>element</code> to the <code>collection</code>, unless the <code>collection</code> already contains
	...
	/**
	 * Searches the <code>searchedElement</code> in the passed <code>collection</code>. The <code>comparator</code> is
	 * used to compare the elements (the comparison result <code>0</code> means the elements are considered equal).
	 */
	...
	/**
	 * Checks if the passed <code>collection</code> contains the <code>searchedElement</code>. See
	 * {@link #getElement(Collection, Object, Comparator)} for more information.
	 */
	...
	/**
	 * Either returns the passed <code>collection</code> or an (unmodifiable) empty collection. Works like
	 * {@link NullSafe#collection(Collection)}.
	 */
	...
	/**
	 * {@link GenericTaskWithContext#perform(Object) Performs} the specified <code>task</code> on the elements of the
	 * passed <code>collection</code>.
	 */
	...
	/**
	 * Returns the first element returned by the <code>collection</code>'s iterator or <code>null</code>, if the
	 * <code>collection</code> is <code>null</code> or empty.
	 */
	...
	/**
	 * {@link #checkSize(Collection, Integer) Checks the size} of the passed <code>collection</code> and then
	 * {@link #getFirstElement(Collection) gets the first element}.
	 */
	...
	public static <E extends Comparable<E>> List<E> getSortedList(final Collection<E> elements) {
		...
		final List<E> list = new ArrayList<E>(elements);
		Collections.sort(list);
		return list;
	}

	/**
	 * Gets a sorted list containing the passed <code>elements</code>. The elements will be sorted using the specified
	...
	public static <E, F> List<F> transform(final Collection<E> elements, Function<E, F> transformer) {
		Arguments.notNull(elements);
		List<F> transformedElements = new ArrayList<F>();
		for (final E element : elements) {
			F transformedElement;
			try {
				transformedElement = transformer.apply(element);
			} catch (Exception e) {
				throw new RuntimeException("Error while transforming element! " + CommonTools.getParametersString("element", element,
						"transformer", transformer, "transformer class", transformer.getClass()), e);
			}
	...
	/**
	 * Basically removes all elements from <tt>s1</tt> which are inside <tt>s2</tt>. The sets being equivalent means it
	 * does not matter against which set we perform the contains check, thus we can chose optimize the implementation
	 * based on sizes of s1 and s2.
	 */
	...
	/**
	 * OK, this might be weird, as this is exactly what the {@link Collection#removeAll(Collection)} method does.
	 * However, the standard implementation of this method for {@link Set} in java has a bug (introduced in
	 * {@link AbstractSet} implementation as an optimization), so the <tt>removeAll</tt> method is not reliable.
	 */
	...
	/**
	 * Returns a list which contains the list given as parameter, except for those on positions given by
	 * <tt>indices</tt>.
	 *
	 * The returned list is a new instance, except for the trivial cases when one of given collections is empty, when it
	 * returns the original list, unmodified.
	 */
	public static <T> List<T> removeByIndices(List<T> list, Collection<Integer> indices) {
		List<Integer> sortedIndices = new ArrayList<Integer>(indices);
	...
	/**
	 * Returns a list which contains the list given as parameter, except for those on positions given by
	 * <tt>indices</tt>, assuming this list is sorted in ascending order.
	 *
	 * The returned list is a new instance, except for the trivial cases when one of given lists is empty, when it
	 * returns the original list, unmodified.
	 */
	...
	/**
	 * Splits a big list of items in multiple separate lists, each
	 * holding maxSize elements. The last resulting list may contain
	 * less items. Note that the resulting lists are subLists of
	 * the original list. Hence, modifications to these lists
	 * are reflected in the original list and vice versa.
	 * 
	 * @param <T> The type of the elements in the list.
	 * @param fullList The full list of all elements to be split.
	 * @param maxSize The maximum size of a single resulting list.
	 * @return A list of lists, each having a maximum of maxSize elements.
	 * @throws IllegalArgumentException When the maxSize value is less or equal 0.
	 */
