...
public class CommonTools {
	...
	/**
	 * Returns <code>true</code> if the passed <code>enumeration</code> is <code>null</code> or is empty (i.e. has no
	 * more elements).
	 */
	 ...
	 *
	 * @return <code>true</code> if the passed <code>string</code> is <code>null</code> or {@link String#isEmpty()
	 *         empty}.
	 */
	 ...
	 *
	 * @return <code>true</code> if the passed <code>string</code> is <code>null</code> or {@link String#isEmpty()
	 *         empty} after {@link String#trim() trimming}.
	 */
	public static boolean isBlank(final String string) {
		/* NOTE: trimming is less efficient than checking the string with Character.isWhiteSpace(). However, that's not
		 * GWT compatible. */
	...
	/**
	 * Returns <code>true</code> if the passed object equals at least one element in the passed collection (i.e. if the
	 * collection {@link Collection#contains(Object) contains} the element). This method just returns <code>false</code>
	...
	/**
	 * Returns <code>true</code> if the passed object equals at least one of the passed <code>objectsToCompareTo</code>
	 * ). This method just returns <code>false</code> if <code>objectsToCompareTo</code> is <code>null</code>.
	 */
	...
	/**
	 * Returns the <code>singular</code> if the specified <code>count</code> is <code>1</code>, otherwise it returns
	 * <code>singular</code> + <code>s</code>.
	 */
	...
	/**
	 * Returns the <code>singular</code> if the specified <code>count</code> is <code>1</code>, otherwise the
	 * <code>plural</code>.
	 */
	...
	/**
	 * Works like {@link #getSingularOrPlural(String, String, int)} except that it adds <code>count</code> (+ space) as
	 * prefix.
	 */
	...
	/**
	 * Returns the passed <code>value</code> or the <code>defaultValue</code> if the <code>value</code> is
	 * <code>null</code>. Works like {@link NullSafe#get(Object, Object)}.
	 */
	...
	/**
	 * Parses the passed string as a <code>boolean</code>. In contrast to {@link Boolean#parseBoolean(String)} a string
	 * that doesn't equal "true" or "false" (ignoring the case) is not parsed as <code>false</code> but an exception is
	 * thrown.
	 *
	 * @param booleanString
	 *            the string to parse.
	...
	/**
	 * Parses the passed string as a <code>boolean</code>. If the string is <code>null</code> or empty, the default
	 * value is returned, otherwise the result of {@link #parseBooleanOrThrowException(String)} is returned.
	 */
	...
	/**
	 * Parses the passed string as a <code>boolean</code>. This method accepts "true", "yes" and "1" as
	 * <code>true</code> and "false", "no" and "0" as <code>false</code>. Furthermore the case is ignored. If the value
	...
	/**
	 * Returns <code>true</code>, if and only if <code>object1</code> and <code>object2</code> are not <code>null</code>
	 * and <code>object1</code> {@link Object#equals(Object) equals} <code>object2</code>, otherwise <code>false</code>.
	 */
	...
	/**
	 * Checks if the passed objects are equal. The method works like {@link #equalsOrBothNull(Object, Object)}. The
	 * method returns <code>true</code>, if both objects are <code>null</code>, because that's also how it's implemented
	 * in collections.
	 */
	...
	/**
	 * Returns <code>true</code>, if and only if both objects are <code>null</code> or if <code>object1</code>
	...
	/**
	 * Returns <code>true</code> if one or both objects are <code>null</code> or if <code>object1</code>
	 * {@link Object#equals(Object) equals} <code>object2</code>, otherwise <code>false</code>.
	 */
	...
	/**
	 * Returns <code>true</code> if (only) one of the passed objects is <code>null</code> or if <code>object1</code>
	 * {@link Object#equals(Object) equals} <code>object2</code>, otherwise <code>false</code>.
	 */
	 ...
	 * If the passed <code>object</code> is a <code>String</code>, quotes are added to the <code>String</code> (--&gt;
	 * 'string'). <br>
	 * Otherwise the result of {@link Object#toString()} is returned.<br>
	 * This method can be used in log lines where one wants to distinguish between a <code>null</code> string, an empty
	 * string (or even the string "null") and between numbers or booleans and strings.
	 *
	 * @param object
	...
	/**
	 * Helper method to get a string that lists method parameters and their actual values (e.g. for error messages). For
	 * example, if a method has two parameters "String someString" and "Integer someNumber" and their values are
	 * <code>"test"</code> and <code>123</code>, one can use this method to get the string
	 * "(someString='someValue',someNumber=123)".
	 *
	 * @param addParentheses
	 *            whether or not to add parentheses to the returned string.
	 * @param parametersAndValues
	 *            1..n pairs of parameter names and values, e.g. "param1, "value1", "param2", "value2", ... , "paramN ",
	 *            "valueN". Alternatively one may also pass a single {@link Map}. In that case the map is converted to
	 *            an object array using {@link MapTools#toArray(Map, Class)}.
	...
	/**
	 * Returns the defaultString if the passed <code>object</code> is <code>null</code>, otherwise the result of
	...
	/**
	 * Returns <code>null</code> if the passed object is <code>null</code>, otherwise the result of
	 * {@link Object#toString()}.
	 */
	...
	/**
	 * Returns an empty string if the passed object is <code>null</code>, otherwise the result of
	 * {@link Object#toString()}.
	 */
	...
	/**
	 * Returns the enum value for the specified <code>enumName</code>. The method at first compares the specified
	 * <code>enumName</code> to all the {@link Class#getEnumConstants() enum constants} of the specified
	 * <code>enumClass</code> using the <code>==</code> operator. If the enum constant is not found that way, the
	 * comparison is performed again using {@link String#equalsIgnoreCase(String)}. Please note that this algorithm is a
	 * bit slower than using {@link Enum#valueOf(Class, String)} if the enum name is (usually) specified correctly (i.e.
	 * case sensitive), but it's faster otherwise. Furthermore the <code>valueOf</code> method would force a stronger
	 * generic restriction.
	 *
	 * @param enumClass
	...
	/**
	 * Returns the passed <code>object</code> or throws a {@link NullPointerException} with the passed
	 * <code>errorMessage</code> if it is <code>null</code>.
	 *
	 ...
	 *             if any of the passed objects is <code>null</code>. The exception message includes the passed
	 *             <code>errorMessage</code> (if set) and the names of objects that are null.
	...
	public static void objsNotNullWithMsg(final String errorMessage, final Object... objectNamesAndObjects) throws NullPointerException {
		...
		final List<String> namesOfObjectsThatAreNull = new ArrayList<String>();
	...
	/**
	 * Converts the first letter of the passed <code>string</code> to upper case. If the string is
	 * {@link #isEmpty(String) empty}, the string is not changed.
	 */
	...
	/**
	 * Converts the first letter of the passed <code>string</code> to lower case. If the string is
	 * {@link #isEmpty(String) empty}, the string is not changed.
	 */
	...
	/**
	 * Creates a map where the keys are the elements of the passed <code>list</code> and the values are the indexes of
	 * the elements in the <code>list</code>.
	 */
	...
	/**
	 * Returns the simple name part of the passed <code>fullyQualifiedClassName</code>, i.e. the sub string after the
	 * last '.'.
	 */
	...
	/**
	 * Returns the package name part of the passed <code>fullyQualifiedClassName</code>, i.e. the sub string before the
	 * last '.'.
	 */
	...
	/**
	 * Returns the class name part of the passed <code>fullyQualifiedClassName</code>, i.e. the sub string after the
	 * last '.' (or the whole string, if it doesn't contain ".").
	 */
	...
	/**
	 * Returns the element with the specified <code>index</code> in the passed <code>array</code>. The
	 * <code>array</code> may be <code>null</code> and the <code>index</code> may be out of bounds. In both cases, the
	...
	/**
	 * Returns the element with the specified <code>index</code> in the passed <code>array</code>. The
	 * <code>collection</code> may be <code>null</code> and the <code>index</code> may be out of bounds. In both cases,
	...
	/**
	 * Returns the same string that the {@link Object#toString() default toString implementation} would return or
	 * "null", if the passed object is <code>null</code>.
	 *
	...
	/**
	 * Similar to {@link #defaultToString(Object)}, but uses the {@link System#identityHashCode(Object) identity hash
	 * code}.
	 */
