...
/**
 * This class provides utility methods related to properties (i.e. which can be accessed via
 * <code>get[PropertyName]</code> and <code>set[PropertyName]</code> methods).
 *
...
public class PropertyTools {
	...
	/**
	 * Gets the value of the specified <code>property</code> for each element of the passed <code>collection</code> and
	...
	/**
	 * Invokes {@link #getPropertyValueByPath(Object, String, boolean, PropertyGetter)} with a
	 * {@link DefaultPropertyGetter}.
	 */
	...
	/**
	 * Gets the value of the property specified by the <code>propertyPath</code>, e.g.
	 * <code>document.version.info</code> (--&gt; <code>getDocument().getVersion().getInfo()</code>).
	 *
	 * @param nullsInPathAllowed
	 *            whether or not a <code>null</code> in the path is allowed, e.g. if it's fine when in
	 *            <code>getDocument().getVersion()</code>, <code>getDocument()</code> returns <code>null</code>.
	 *            Otherwise an exception is thrown.
	 * @param propertyGetter
	 *            used to get the values of the properties in the <code>propertyPath</code>. Default implementation is
	 *            {@link DefaultPropertyGetter}.
	...
	/**
	 * Returns the property path after the {@link #getFirstProperty(String) first property}, e.g. "property2.property3"
	 * if path is "property1.property2.property3".
	 */
	...
	/**
	 * Invokes {@link #getListSortedByProperties(PropertyGetter, Collection, SortProperty...)} with a
	 * {@link DefaultPropertyGetter}.
	 */
	...
	/**
	 * Works {@link #sortByProperties(PropertyGetter, List, SortProperty...)} except that the elements from the
	 * <code>collection</code> are first put into a new <code>List</code>. The passed <code>collection</code> will not
	 * be modified.
	 */
	public static <T> List<T> getListSortedByProperties(final PropertyGetter propertyGetter, final Collection<T> collection,
		...
		final ArrayList<T> list = new ArrayList<T>(collection);
	...
	/**
	 * Default implementation of {@link PropertyGetter}. Uses {@link PropertyTools#getPropertyValue(Object, String)} to
