...
/**
 * This is a helper class that is used by {@link FileTools#deleteFileWhenOrphaned(File)}.
 * Please read the description of the {@link #deleteFileWhenOrphaned(File)} method carefully.
 */
public class FileAutoDeletion implements AutoCloseable {
	 ...
	 * This method takes as a parameter the file that should be deleted once there is not reference to it anymore.
	 * This should always be a fallback mechanism as files should be deleted as soon as they are not needed
	 * anymore. There are, however, circumstances where this is out of the control of the code that
	 * creates the file.
	 * So, when the provided File object is no longer referenced (i.e., it is orphaned and about to be garbage collected),
	 * this code will make sure that the file gets deleted first.
	 * This should only be used for temporary files!
	 * A word of caution: use this method with meticulous care. If, for example, a FileInputStream is constructed on this file,
	 * this FileInputStream does NOT hold a reference to this file. Hence, this would mean that the file could
	 * get deleted while there is still an FileInputStream open on this file. If in doubt, don't use this method.
	 * 
	 * @param tempFile The file that should be monitored for getting an orphan.
	...
	/**
	 * Initializes the shutdown hook and the cleanup thread that are necessary to delete orphaned files.
	 * This initialization only happens once. Hence, this method can be called multiple times with
	 * only taking effect once.
	 */
