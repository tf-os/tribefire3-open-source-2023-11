...
public final class DateTools {
	...
	/**
	 * The default {@link java.text.SimpleDateFormat#SimpleDateFormat(String) date format pattern}:
	 * <code>yyyy.MM.dd HH:mm:ss.SSS</code>. This pattern was also used as default in the I2Z server code. Please note
	 * that the pattern also parses <code>yyyy.MM.dd HH:mm:ss.SS</code> and <code>yyyy.MM.dd HH:mm:ss.SSS</code>
	 * correctly.
	 */
	public static final String DEFAULT_DATEFORMAT_PATTERN = "yyyy.MM.dd HH:mm:ss.SSS";

	/**
	 * The supported date/time formats that are used by {@link #parseDate(String)} to parse a date string. The array is
	 * initialized in the static initializer block.
	 */
	private static final DateTimeFormatter[] dateTimeFormatParsers;
	/**
	 * The supported date formats that are used by {@link #parseDate(String)} to parse a date string. The array is
	 * initialized in the static initializer block.
	 */
	...
	/**
	 * Parses the passed <code>dateString</code> and returns the date. Parse exceptions are not logged, just ignored.
	 * The <code>DateFormat</code> instances are created during initialization of the class.
	 *
	 * @param dateString
	 *            the string to parse.
	...
	/**
	 * Computes the time in another time zone. For more information on {@link TimeZone}s and the supported IDs please
	 * check http://java.sun.com/j2se/1.5.0/docs/api/java/util/TimeZone.html.
	 *
	 * @param localTime
	 *            the time to convert.
	...
	/**
	 * Converts the passed <code>date</code> to a string using the specified <code>simpleDateFormatPattern</code>.
	 * Milliseconds are handled as decimals. See {@link #getDateString(Date, String, boolean)} for more information.
	 */
	 ...
	 * <p>
	 * Explanation:<br>
	 * <code>SimpleDateFormat</code> works as follows: if the date/time is "1 second and 12 milliseconds" and the
	 * {@link java.text.SimpleDateFormat} pattern is "s.SS", the result will be "1.12" (instead of "1.01"), because the
	 * number of "S" characters in the pattern only specifies the minimum number of milliseconds digits. This also means
	 * that for "1 second and 1 millisecond" the result would be "1.01". Furthermore, because it's just the MINIMUM
	 * number of digits, so for "1 second and 123 milliseconds" the result would be "1.123" instead of "1.12". These are
	 * not bugs but in many cases it's not what the programmer expects and wants.
	 * </p>
	 * <p>
	 * If <code>millisecondsHandledAsDecimals</code> is set to <code>true</code> the milliseconds characters ("S") in
	 * the pattern are handled as decimals, i.e. as a fraction of a second. This means that the method always returns
	 * the number of requested digits. For "1 second and 12 milliseconds" and pattern "s.SS" the result is "1.01", for
	 * "s.SSS" it's "1.012" for "s.SSSSS" it's 1.01200". Please note that for "1 second and 19 milliseconds " and
	 * pattern "s.SS" the method returns "1.01" and not "1.02" (i.e. numbers are not rounded).
	 * </p>
	 *
	 * @param date
	...
	public static String getDateString(final Date date, final String simpleDateFormatPattern, final boolean millisecondsHandledAsDecimals) {
		if (!millisecondsHandledAsDecimals) {
			/* NOTE: To improve performance date format instances could be cached. (see e.g.
			 * biz.i2z.utils.format.FormatProvider) */
		...
		if (millisecondsDateString.length() > requestedMillisecondsDigitsCount) {
			...
			/* we need more digits! since we already prepended zeros to the string, there can only be further missing
			 * digits if the pattern is something like "s.SSSSS", i.e. more than 3 milliseconds digits. In this case we
			 * just append zeros to the string. */
	...
	/**
	 * Simple helper method that returns a number based on the current date and time. It is NOT guaranteed that this
	 * number is unique!
	 */
	...
	/**
	 * Utility method to parse time spans in a human-readable form and returns the corresponding time interval in
	 * milliseconds. The time span is specified in one or more groups of values and time units. Supported time units
	 * are: y (year), m (month), d (day), h (hour), min (minute), s (second) and ms (millisecond). Please note that this
	 * code does not prevent the resulting long value to become negative if the time span exceeds the maximum long
	 * value.
	 * 
	 * @param timeSpanSpec
	 *            The string containing the time span specification
	 * @param allowedUnits
	 *            An optional set of time units that are allowed. If this is not provided, or empty, all time units will
	 *            be used.
	 * @throws IllegalArgumentException
	 *             Thrown when the time span specification is null, or when it does not contain an even number of
	 *             value/units pairs.
