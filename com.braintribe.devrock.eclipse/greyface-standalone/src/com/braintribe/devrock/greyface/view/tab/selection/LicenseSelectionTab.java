...
public class LicenseSelectionTab extends GenericViewTab implements Listener, HasTreeTokens, ScanProcessListener, SelectionProcessListener, 
	...
	private Map<String, TreeItem> licenseToItemMap = new HashMap<String, TreeItem>();
	private TreeItem noLicenseItem;
	...
	public LicenseSelectionTab(Display display) {
		super(display);
	...
	protected Composite createControl(Composite parent) {
		...
		licenseTree = new Tree ( treeComposite, SWT.MULTI | SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
		licenseTree.setHeaderVisible( true);
		licenseTree.setLayout(scanLayout);
		licenseTree.setLayoutData( new GridData( SWT.FILL, SWT.FILL, true, true, 4, 4));
		...
		for (int i = 0; i < targetColumnNames.length; i++) {
			TreeColumn treeColumn = new TreeColumn( licenseTree, SWT.LEFT);
		...
		flatTreeColumnResizer.setTree( licenseTree);		
		licenseTree.addControlListener(flatTreeColumnResizer);
		
		licenseTree.addListener(SWT.Expand, this);
		licenseTree.addListener( SWT.MouseDoubleClick, this);
		
		TreeItemTooltipProvider.attach(licenseTree, KEY_TOOLTIP);
	...
	private void handleSelection( Event event){
        ...
        TreeItem item = licenseTree.getItem(point);
        if (item == null)
        	return;       
	...
	public void acknowledgeScannedArtifact( final RepositorySetting setting, final Solution artifact, final Set<Artifact> parents, final boolean presentInTarget) {
		...
		display.asyncExec( new Runnable() { 
			@Override			
			public void run() {								
				Set<TreeItem> parents = acquireLicenseParentItems(artifact);
				for (TreeItem parent : parents) {
					TreeItem item = TreeItemHelper.createItem( parent, items, itemToDuplicates, setting, artifact);					
				...
				// link all projections.. if multiple
			}
		});		
	}
	...
	private TreeItem acquireLicenseTreeItem( String license) {
		TreeItem item = licenseToItemMap.get(license);
		if (item == null) {
			item = new TreeItem( licenseTree, SWT.NONE);
			item.setText( license);
			licenseToItemMap.put(license, item);
			
		}
		return item;
	}
	
	private Set<TreeItem> acquireLicenseParentItems( final Solution artifact) {
		Set<License> licenses = artifact.getLicenses();
	
		if (licenses.size() == 0) {
			TreeItem parent = acquireLicenseTreeItem(NO_LICENSE);
			noLicenseItem = parent;
			return Collections.singleton( parent);
		}
		Set<TreeItem> parents = new HashSet<TreeItem>();
		for (License license : licenses) {			
			String name = license.getName();
			parents.add( acquireLicenseTreeItem(name));
		}
	...
	public void acknowledgeScanAbortedAsArtifactIsPresentInTarget( final RepositorySetting target, final Solution artifact, final Set<Artifact> parents) {
		...
		display.asyncExec( new Runnable() { 
			...
			public void run() {
				Set<TreeItem> parents = acquireLicenseParentItems(artifact);
				for (TreeItem parent : parents) {								
					TreeItem item = TreeItemHelper.createItem( parent, items, itemToDuplicates, target, artifact);																				
					TreeItem parts = TreeItemHelper.attachParts(item, artifact);
					if (parts != null)
						parts.setImage(partsImage);
					item.setFont( italicFont);
	...
	public void acknowledgeUnresolvedArtifact(final List<RepositorySetting> sources, final Dependency dependency, final Collection<Artifact> requestors) {
		...
		display.asyncExec( new Runnable() { 
			@Override			
			public void run() {								
				TreeItem item = TreeItemHelper.createItem( licenseTree, items, itemToDuplicates, null, dependency);		
	...
	public void acknowledgeStartScan() {
		display.asyncExec( new Runnable() { 
			@Override
			public void run() {
				licenseTree.removeAll();
				items.clear();
				itemToDuplicates.clear();
				listed.clear();
				licenseToItemMap.clear();
				noLicenseItem = null;
			}
		});
	...
	public void acknowledgeStopScan() {		
		display.asyncExec( new Runnable() {
			...
			public void run() {
					...
					// look at the artifacts in no license section, and drill down to the parents to find if
					// they really don't have a license (inherited)
					if (noLicenseItem == null) {
						return;
					}
					Map<TreeItem, TreeItem> reassignMap = new HashMap<>();
	
					for (TreeItem suspectItem : noLicenseItem.getItems()) {
						Artifact artifact = (Artifact) suspectItem.getData( KEY_ARTIFACT);
						if (artifact == null) {
							continue;
						}
						Solution solution = artifact.getResolvedParent();
						if (solution == null) {
							continue;
						}
						boolean done = false;
						do {
							...
							if (licenses.size() != 0) {
								for (License license : licenses) {
									TreeItem licenseParentItem = licenseToItemMap.get(license.getName());
									if (licenseParentItem != null) {
										reassignMap.put(suspectItem, licenseParentItem);
										System.out.println("assigning license via parent : [" + NameParser.buildName(artifact) + "] <- [" + NameParser.buildName(solution) + "]");			
									}
									else {
										System.out.println("unknown license via parent : [" + NameParser.buildName(artifact) + "] <- [" + NameParser.buildName(solution) + "]");
									}
								}
								done = true;
							}
							if (!done) {
								solution = solution.getResolvedParent();
							}
							
							if (!done) {
								if (solution == null) {
									System.out.println("no license found for [" + NameParser.buildName(artifact) + "]");
									break;
								}
								else {
									System.out.println("trying for [" + NameParser.buildName(artifact) + "] next parent [" + NameParser.buildName(solution) + "]");
								}															
							}						
						} while (!done);
					...
					// reassign
					for (Entry<TreeItem, TreeItem> entry : reassignMap.entrySet()) {
						TreeItem item = new TreeItem( entry.getValue(), SWT.NONE);
						transfer( entry.getKey(), item);
						entry.getKey().dispose();
					}
					// if no unknown licenses remain, remove the item
					if (noLicenseItem.getItemCount() == 0) {
						noLicenseItem.dispose();
						noLicenseItem = null;
					}
			}
		});
	...
	private TreeItem transfer( TreeItem source, TreeItem target) {
		target.setText( source.getText());
		target.setImage( source.getImage());
		
		String [] keys = new String [] {KEY_DEFERRED, KEY_REPOSITORY, KEY_ARTIFACT, 
										KEY_PART, KEY_ROOT, KEY_SELECTION, KEY_NAME, KEY_TOOLTIP, KEY_SCAN_END,
										KEY_EXISTS_IN_TARGET, KEY_REPOSITORY_SETTING, KEY_ACTIVE, KEY_UNRESOLVED};
		for (String key : keys) {
			Object value = source.getData( key);
			if (value != null) {
				target.setData(key, value);
			}
		}
		TreeItem [] sourceChildren = source.getItems();
		if (sourceChildren != null) {
			for (TreeItem sourceChild : sourceChildren) {
				TreeItem targetChild = new TreeItem( target, SWT.NONE);
				transfer( sourceChild, targetChild);				
			}
		}
		return target;
	}
	...
	public void acknowledgeArtifactSelected(Solution solution) {				
		Set<TreeItem> items = collectAllItems();
	...
	private Set<TreeItem> collectAllItems() {
		Set<TreeItem> items = new HashSet<TreeItem>();
		TreeItem[] licenseItems = licenseTree.getItems();
		for (TreeItem licenseItem : licenseItems) {
			if (licenseItem.getItemCount() > 0) {
				items.addAll( Arrays.asList( licenseItem.getItems()));						
			}
			else {
	...
	public void widgetSelected(SelectionEvent e) {		
		...
		if (e.widget == selectRelated) {
			// 
			TreeItem [] items = licenseTree.getSelection();
	...
	public void acknowledgeFailedPart(RepositorySetting setting, final Solution solution, final Part part, String reason, int worked) {
		display.asyncExec( new Runnable() { 
			@Override
			public void run() {						
				List<TreeItem> items = TreeItemHelper.collectTreeItems(licenseTree);			
	...
	public Set<Part> getSelectedParts() {
		...
		if (!context.getRepairExistingInTarget())
			return parts;		
		for (TreeItem item : TreeItemHelper.collectTreeItems(licenseTree)) {		
			if (Boolean.TRUE.equals( item.getData( KEY_SELECTION))) {
				Part part = (Part) item.getData( KEY_PART);
				if (part != null)
					parts.add( part);
	...
	@Override
	public void expand() {
		treeExpander.expandTree(licenseTree, true);
		
	}

	@Override
	public void condense() {
		treeExpander.expandTree(licenseTree, false);		
	}
	...
	public void selectAll() {
		for (TreeItem item : licenseTree.getItems()) {
			if (Boolean.TRUE.equals(item.getData( KEY_EXISTS_IN_TARGET))) {
				if (!context.getOverwriteExistingInTarget()) {
					continue;
				}
	...
	public String copyContents() {
		...
		for (TreeItem licenseItem : licenseItems) {
			...
			if (Boolean.TRUE.equals( licenseItem.getData(KEY_UNRESOLVED))) {
				mainBuilder.append( "\n" + licenseItem.getText());
				mainBuilder.append( "\t UNRESOLVED DEPENDENCY");;
			}
			else {
				mainBuilder.append( licenseItem.getText());
			}
			mainBuilder.append("\n");
			TreeItem [] items = licenseItem.getItems();
			...
			for (TreeItem item : items) {				
				...
				itemBuilder.append( "\t\t");
				itemBuilder.append( item.getData( KEY_TOOLTIP));
				itemBuilder.append( "\n");
				Solution solution = (Solution) item.getData( KEY_ARTIFACT);
				itemBuilder.append(ClipboardCopier.processArtifact(solution, i+2));				
			}
			mainBuilder.append( itemBuilder.toString());
		}
