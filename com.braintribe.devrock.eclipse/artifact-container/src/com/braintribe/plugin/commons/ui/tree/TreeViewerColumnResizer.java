...
public class TreeViewerColumnResizer extends ControlAdapter {
	...
	public void setColumns(List<TreeViewerColumn> columns) {
		...
		this.columnWeights = new int[ columns.size()];
		for (int i = 0; i < columnWeights.length; i++) {
			columnWeights[i] = columns.get(i).getColumn().getWidth();
	...
	private void recalculateColumns(int width) { 				 
    	 ...
    	 for (int i = 0; i < columns.size(); i++) {
    		TreeViewerColumn column = columns.get(i);
    		 column.getColumn().setWidth( (int) (widthPerColumn * columnWeights[i]));
    	 }
	 }
