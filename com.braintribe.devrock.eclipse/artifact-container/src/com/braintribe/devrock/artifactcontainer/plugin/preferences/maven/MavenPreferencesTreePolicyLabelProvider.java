...
public class MavenPreferencesTreePolicyLabelProvider extends LabelProvider implements IStyledLabelProvider, ToolTipProvider {
	...
	private Image noUpdatePolicyImage;
	...
	public MavenPreferencesTreePolicyLabelProvider(MavenPreferencesTreeRegistry registry) {
		...
		imageDescriptor = ImageDescriptor.createFromFile( getClass(), "participant_rem.gif");
		noUpdatePolicyImage = imageDescriptor.createImage();
	...
	public StyledString getStyledText(Object element) {
		if (element instanceof Repository) {
			...
			if (registry.getDynamicUpdatePolicySupport(repository)) {
				...
				if (
						(snapshotPolicy == null || !snapshotPolicy.getEnabled()) &&
						(releasePolicy == null || !releasePolicy.getEnabled()) 
				   ) {					
					return new StyledString( "inactive");
				}
				else {
					return new StyledString( "maven");
				}				
			}
		}
	...
	public void dispose() {
		...
		if (noUpdatePolicyImage != null) 
			noUpdatePolicyImage.dispose();
		...
		if (element instanceof Repository) {
			...
			if (registry.getDynamicUpdatePolicySupport(repository)) {
				...
				if (
					...
					return new String("no update policy");
				}
				else {
