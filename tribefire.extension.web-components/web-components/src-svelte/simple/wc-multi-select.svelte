<svelte:options tag="wc-multi-select" />
...
<script>
  ...
  function handleKeyup(e) {
    if (e.keyCode === 13) {
      // dodaj novu opciju i aktivnu opciju stavi da je newOption
      if (!activeOption) {
        const newOption = { value: inputValue.trim(), name: inputValue.trim() };
        options.push(newOption);
        // selected[inputValue] = newOption;
  ...
  {#if showOptions}
    ...
    >
      ...
      {#if filtered.length <= 0 && inputValue && inputValue.length > 0}
        <li>Pres enter to ad new option!</li>
      {/if}
    </ul>
  {/if}
</div>
