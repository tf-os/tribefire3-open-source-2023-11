  ...
  function handleKeyChanged(prevKey: string, event: FocusEvent) {
    ...
    if (prevKey === null) {
      if (nextKey) {
        ...
        dispatchEvent("create_entry", { key: nextKey, value: nextValue });
      }
      tableCell.innerHTML = "";
    } else if (!nextKey) {
      // delete key
...
<main>
  <div class="toolbar">
    ...
    <div class="search-box">
      ...
      />
      {#if query}
        <label class="clear-search" on:click={() => (query = "")} />
      {/if}
    </div>
  </div>
  {#if entries}
    <div class="form-wrapper" class:hidden={mode !== "form"}>
      <table>
        {#each formChunks as { key, keyChunks, valueChunks } (key)}
          <tr>
        ...
        {:else if !entries || entries.length === 0}
          <tr>
            <td>
              It seems that there are no items yet and your user account does
              not have sufficient privileges to create one.
            </td>
          </tr>
        {/if}
      </table>
...
<style type="text/scss">
  * {
  ...
  tr {
    ...
    &.add-new-item-row:not(:focus-within) {
