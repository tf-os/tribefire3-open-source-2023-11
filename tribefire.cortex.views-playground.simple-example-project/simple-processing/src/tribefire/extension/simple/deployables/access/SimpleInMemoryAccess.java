...
/**
 * A simple {@link IncrementalAccess} implementation that stores entities from the <code>simple-data-model</code> in
 * memory. The implementation is based on {@link BasicAccessAdapter}.<br>
 * The {@link #loadPopulation() population} can {@link #isInitializeWithExampleData() optionally} be initialized with
 * {@link #createExampleData() example data}.
 *
...
public class SimpleInMemoryAccess extends BasicAccessAdapter implements InitializationAware {
	...
	/**
	 * Initializes this access. Dependent on {@link #isInitializeWithExampleData()} this also
	 * {@link #createExampleData() creates example data}.<br>
	 * This method is called automatically after creation of all deployables (see
	 * {@link InitializationAware#postConstruct()}).
	 */
	@Override
	 ...
	 * {@link com.braintribe.model.access.BasicAccessAdapter.AdapterManipulationReport#getUpdatedEntities() updated}
	 * entities.
	 * <p>
	 * Note that this (simple) implementation does not support all features, for example entity type and collection
	...
	/**
	 * Creates some example data, i.e. a {@link Company} with a few {@link Department}s and some {@link Person}s
	 * {@link Company#getEmployees() employed at} that company.
	 */
	private void createExampleData() {
		...
		storage.forEach((entity) -> setIdUnlessSet(entity));
