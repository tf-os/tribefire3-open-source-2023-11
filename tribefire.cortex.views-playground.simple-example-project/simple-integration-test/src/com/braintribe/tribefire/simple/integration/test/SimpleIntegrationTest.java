 ...
 * The main purpose of this class is to demonstrate how to write integration tests for cartridges and tribefire in
 * general.<br>
 * <p>
 * In contrast to a unit test, which first must set up the unit(s) to be tested (e.g. an access), this integration test
 * expects a prepared tribefire-services instance to be available and just {@link #newRemoteSession() opens a remote
 * session} to it. After obtaining the session the actual test code is simple and similar to a normal unit test.
 * <p>
 * The test requires a prepared environment, i.e. running tribefire-services with simple-cartridge and deployables set
 * The tribefire tool stack provides the tools <i>Jinni</i> and <i>tfcloud-cli</>, which can be used to create such an
 * environment based on <code>simple-cartridge-setup</code>, either locally or in a Kubernetes cluster. These tools can
 * also easily be integrated into a CI pipeline. For further information on those tools, please check the respective
 * documentation.
 * <p>
 *
...
public class SimpleIntegrationTest extends AbstractTest {
	...
	/**
	 * The default tribefire-services url, used by {@link #newRemoteSession()} unless
	 * {@value #TRIBEFIRE_SERVICES_URL_SYSTEM_PROPERTY} is set.
	 */
	...
	/**
	 * This method first tries to get the tribefire-services url from system property
	 * {@value #TRIBEFIRE_SERVICES_URL_SYSTEM_PROPERTY} (which can e.g. be provided by CI pipeline). If the property is
	 * not set, the default url {@value #DEFAULT_TRIBEFIRE_SERVICES_URL} is used. Afterwards the method just delegates
	 * to {@link #newRemoteSession(String)}.
	 *
	 */
	...
	/**
	 * Connects to tribefire services via the specified specified <code>servicesUrl</code> using the passed credentials
	 * and returns a session which can be used to access the <code>impleInMemoryAccess</code>, but also to send service
	 * requests.
	 */
	...
	/**
	 * A simple example test for the <code>SimpleInMemoryAccess</code>. It just runs a simple query and then checks the
	 * results.
	 */
	@Test
	...
	/**
	 * A simple example test for the <code>SimpleEchoService</code>. It sends a {@link SimpleEchoRequest} and then
	 * checks the {@link SimpleEchoResponse}.
	 */
	@Test
