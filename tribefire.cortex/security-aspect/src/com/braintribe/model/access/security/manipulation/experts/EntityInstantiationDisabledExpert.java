...
/**
 * {@link ManipulationSecurityExpert} for {@link Instantiable} constraint.
 */
public class EntityInstantiationDisabledExpert implements ManipulationSecurityExpert {
	...
	public void expose(ManipulationSecurityExpositionContext context) {
		...
		if (context.getCurrentManipulationType() == ManipulationType.INSTANTIATION) {
			if (!instantiable(context.getTargetReference().getTypeSignature(), context.getCmdResolver())) {
				...
				violationEntry.setDescription("[EntityNotInstantiable] Entity: " + context.getTargetReference().getTypeSignature());
	...
	protected boolean instantiable(String typeSignature, CmdResolver cmdResolver) {
		return cmdResolver.getMetaData().entityTypeSignature(typeSignature).is(Instantiable.T);
	}
