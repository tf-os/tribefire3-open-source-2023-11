...
public class Validator {
	 ...
	 */
	private void validate(RepoletContent repoletContent, Artifact expectedArtifact, AnalysisArtifact foundArtifact, boolean validateParts) {
		...
		// parent
		validateParentStructure(repoletContent, expectedArtifact, foundArtifact);
	...
	/**
	 * validate an artifact's parent information, parent plus import
	 * @param expectedArtifact
	 * @param foundArtifact
	 */
	private void validateParentStructure(RepoletContent repoletContent, Artifact expectedArtifact, AnalysisArtifact foundArtifact) {	
		VersionedArtifactIdentification expectedParentVai = expectedArtifact.getParent();		
		AnalysisDependency foundParentDependency = foundArtifact.getParent();
		if (expectedParentVai == null && foundParentDependency == null) 
			return;
		
		if (expectedParentVai == null && foundParentDependency != null) {
			insertAssertionMessage("didn't expect a parent, but found [" + foundParentDependency.asString() + "]");
			return;
		}
			
		Artifact expectedParentArtifact = repoletContent.getArtifacts().stream().filter( a -> expectedParentVai.compareTo(a) == 0).findFirst().orElse(null);
		if (expectedParentArtifact == null) {
			log.warn("no parent found in passed repolet content. No validation will take place");
			return;
		}
		AnalysisArtifact foundParentArtifact = foundParentDependency.getSolution();
						
		if (!assertTrue( "expected parent to be [" + expectedParentVai.asString() + "], yet found none", foundParentArtifact != null)) {
			return;
		}
		
		assertTrue( "expected parent to be [" + expectedParentVai.asString() + "], yet found ["+ foundParentArtifact.asString() + "]", expectedParentVai.compareTo(foundParentArtifact) == 0);
		
		List<AnalysisDependency> foundImports = foundArtifact.getImports();
		List<Dependency> expectedImportDependencies = expectedParentArtifact.getManagedDependencies().stream().filter( d -> d.getScope().equals("import")).collect( Collectors.toList());
		
		for (Dependency expectedImport : expectedImportDependencies) {
			AnalysisDependency matchingImportDependency = foundImports.stream().filter( ad -> ad.compareTo( expectedImport) == 0).findFirst().orElse(null);
			if (!assertTrue("expected import dependency to be [" + expectedImport.asString() + "], yet didn't find it", matchingImportDependency != null)) {
				continue;
			}
			AnalysisArtifact declarator = matchingImportDependency.getDeclarator();		
			assertTrue( "expected declarator of [" + matchingImportDependency.asString() + "] to be [" + foundArtifact.asString() + "], yet it's [" + declarator.asString() + "]", foundParentArtifact.compareTo(declarator) == 0);
		}
		
				
		validateParentStructure(repoletContent, expectedParentArtifact, foundParentArtifact);
		...
		for (AnalysisTerminal terminal : terminals) {
			if (terminal instanceof AnalysisArtifact) {
				...
				if (match == null) {
					...
					matching.add(match);
					validate( repoletContent, match, suspect, validateParts);	
				}											
			}
		}
	...
	private void validateSolutions(RepoletContent repoletContent, AnalysisArtifactResolution resolution, boolean validateParts) {
		...
		for (Artifact expected : expectedArtifacts) {	
			if (expected.getDisregardAsSolutionInResolution()) {
				continue;
			}
			...
			if (found == null) {
				...
				matching.add(found);
				validate(repoletContent, expected, found, validateParts);							
			}			
		}
	...
	public void validateFailedResolution( AnalysisArtifactResolution resolution, List<String> expectedIncompleteArtifacts, List<String> expectedUnresolvedDependencies) {
		...
		if (expectedIncompleteArtifacts != null && !expectedIncompleteArtifacts.isEmpty()) {
			...
			List<String> foundIncompleteArtifacts = resolution.getIncompleteArtifacts().stream().filter( a -> a != null).map( a -> a.asString()).collect( Collectors.toList());
	...
	/**
	 * compares two consumable artifacts with all parts, plus checks hashes (after upload/copy)
	 * @param source - the {@link com.braintribe.model.artifact.consumable.Artifact} before  
	 * @param target - the {@link com.braintribe.model.artifact.consumable.Artifact}
	 * @param expectHashes - true if hash-parts are to be expected and compared (.md5,.sha1,.sha256)
	 */
	...
	/**
	 * simple compares the content of {@link AnalysisArtifact} to a list of names (stringified {@link AnalysisArtifact}), 
	 * without order
	 * @param solutions - the {@link List} of {@link AnalysisArtifact}
	 * @param expectedNames - a {@link List} of {@link String} containing the expected names. 
	 */
