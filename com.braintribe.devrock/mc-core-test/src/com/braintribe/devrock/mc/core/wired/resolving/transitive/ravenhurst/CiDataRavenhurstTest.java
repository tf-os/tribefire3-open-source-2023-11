...
public class CiDataRavenhurstTest extends AbstractRavenhurstTest {
	...
	public void runTest() {
		...
		try {
			copyAndPatch( new File(input, "last-changes-access-archive.ci.yaml"), "last-changes-access-archive.yaml");
			run( "com.braintribe.devrock.test:t#1.0.1", standardTransitiveResolutionContext, true);
		} catch (Exception e) {
			exceptionThrown = true;
		}
		if (exceptionThrown) {
			Assert.fail("expected exception is thrown");			
		}
	}
