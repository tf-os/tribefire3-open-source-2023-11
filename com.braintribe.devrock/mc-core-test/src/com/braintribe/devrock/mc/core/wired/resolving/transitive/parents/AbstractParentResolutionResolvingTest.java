...
public abstract class AbstractParentResolutionResolvingTest implements HasCommonFilesystemNode {
	...
	{	
		Pair<File,File> pair = filesystemRoots("wired/transitive/parents");
		input = pair.first;
		output = pair.second;
	...
	protected AnalysisArtifactResolution run(String terminal, ClasspathResolutionContext resolutionContext, boolean expectException, String suffix) {
		...
		try (				
			...
			String name = buildDumpFilename(terminal, suffix);
	...
	protected String buildDumpFilename(String terminal, String suffix) {
		String sanitizedTerminal = terminal.replace( ":", ".");
		String name = suffix.length() > 0 ? sanitizedTerminal + "-" + suffix : sanitizedTerminal;		
