...
/**
 * tests how mc-core reacts if it detects OUTDATED files (marked) in a OFFLINE state
 * marked maven-metadata files are to remain intact 
 * marked part-availability files are to be processed (as it's just a clean up)
 * @author pit
 *
 */
@Category(KnownIssue.class)
public class PreparedOfflineCompoundResolvingWithRavenhurstTest implements LauncherTrait,HasCommonFilesystemNode {
	private static Logger log = Logger.getLogger(PreparedOfflineCompoundResolvingWithRavenhurstTest.class);	
	...
	private List<File> markerFilesToStillExist; 
	{
		markerFilesToStillExist = new ArrayList<>();
		
		markerFilesToStillExist.add( ArtifactAddressBuilder.build().root( localRepository.getAbsolutePath()).groupId(grp).artifactId(art).file("maven-metadata-archiveA.xml.outdated").toPath().toFile());
		markerFilesToStillExist.add( ArtifactAddressBuilder.build().root( localRepository.getAbsolutePath()).groupId(grp).artifactId(art).file("maven-metadata-archiveB.xml.outdated").toPath().toFile());
	}
		
	
	private List<File> markerFilesNotToStillExist; 
	{
		markerFilesNotToStillExist = new ArrayList<>();
		markerFilesNotToStillExist.add( ArtifactAddressBuilder.build().root( localRepository.getAbsolutePath()).groupId(grp).artifactId(art).version("1.0").file("part-availability-archiveA.txt.outdated").toPath().toFile());
		markerFilesNotToStillExist.add( ArtifactAddressBuilder.build().root( localRepository.getAbsolutePath()).groupId(grp).artifactId(art).version("2.0").file("part-availability-archiveB.txt.outdated").toPath().toFile());		
	}
	protected String getRoot() {
		return "wired/ravenhurst.resolving.offline";
	}
	...
	public void resolvingTest() throws Exception {
		...
		try (
			...
			for (File file : markerFilesToStillExist) {
				Assert.assertTrue("expected marker file [" + file.getAbsolutePath() + "] to still exist", file.exists());
			}
			for (File file : markerFilesNotToStillExist) {
				Assert.assertTrue("expected marker file [" + file.getAbsolutePath() + "] not to still exist", !file.exists());
			}
