...
/**
 * test redirection .. artifact#1.0 -> artifact#2.0 -> artifact#3.0
 * - dependency resolver : standard behavior
 * - part resolver : standard behavior
 * - artifact resolver : redirections, 1.0 -> 3.0, 2.0 -> 3.0, 3.0 -> 3.0
 *  
 * @author pit
 *
 */
public class RedirectedCompoundResolvingTest implements LauncherTrait, HasCommonFilesystemNode {
	private static Logger log = Logger.getLogger(RedirectedCompoundResolvingTest.class);	
	...
	protected String getRoot() {
		return "wired/resolving.redirection";
	}
	...
	private void runArtifactResolverTest(ArtifactDataResolverContract artifactDataResolverContract) {
		...
		Assert.assertTrue("expected [" + cais.get(0).asString() + "], found [" + resolvedCai1.get().asString() +"]", resolvedCai1.get().compareTo( cais.get(2)) == 0);
		...
		Assert.assertTrue("expected [" + cais.get(1).asString() + "], found [" + resolvedCai2.get().asString() +"]", resolvedCai2.get().compareTo( cais.get(2)) == 0);
