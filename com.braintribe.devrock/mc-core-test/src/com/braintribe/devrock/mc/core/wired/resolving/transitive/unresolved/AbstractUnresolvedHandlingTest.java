...
/**
 * abstract common base for all tests working with 'unresolved' dependencies, including 
 * redirection targets, parents and imports. 
 * Each test should test both the TDR and the CPR - just be sure
 * @author pit
 *
 */
public abstract class AbstractUnresolvedHandlingTest implements HasCommonFilesystemNode {
	...
	{	
		Pair<File,File> pair = filesystemRoots("wired/transitive/unresolved");
		input = pair.first;
		output = pair.second;
	...
	protected ClasspathResolutionContext lenientClasspathResolutionContext = ClasspathResolutionContext.build().lenient(true).done();
