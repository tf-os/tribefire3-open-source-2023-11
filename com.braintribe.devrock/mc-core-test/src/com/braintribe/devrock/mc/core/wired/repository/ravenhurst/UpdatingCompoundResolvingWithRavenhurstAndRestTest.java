...
public class UpdatingCompoundResolvingWithRavenhurstAndRestTest implements LauncherTrait, RepoletUsingTrait, HasCommonFilesystemNode {
	private static Logger log = Logger.getLogger(UpdatingCompoundResolvingWithRavenhurstAndRestTest.class);
	...
	protected String getRoot() {
		return "wired/ravenhurst.updating.rest";
	}
	...
	private Launcher launcher = Launcher.build()
					.repolet()
						...
						.serverIdentification("Artifactory/repolet")
						.indexedFilesystem()			
							.initialIndex("one")
	...
	List<Pair<String,CompiledArtifactIdentification>> artifactExpectations = new ArrayList<>();
	{
		...
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + art + "#1.0")));
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + art + "#2.0")));
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + artA + "#1.0.1")));
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + artB + "#1.0.1")));
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + artC + "#1.0.1")));
		artifactExpectations.add( Pair.of( "two", CompiledArtifactIdentification.parse(grp + ":" + artD + "#1.0.1")));
	...
	private void runPartResolvingTest(String stage, ArtifactDataResolverContract artifactDataResolverContract) {
		...
		for (Pair<String, Pair<CompiledArtifactIdentification, Pair<PartIdentification, Boolean>>> pair : partExpectations) {
			...
			if (piToPathPair.second) {
				path = ArtifactAddressBuilder.build().root(repo.getAbsolutePath()).compiledArtifact(cai).part(pi).toPath().toFilePath();
			}
