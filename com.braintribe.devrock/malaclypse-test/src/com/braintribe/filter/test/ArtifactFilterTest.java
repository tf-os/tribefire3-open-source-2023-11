...
public class ArtifactFilterTest implements LauncherTrait {

	private String walkScopeId = UUID.randomUUID().toString();
	private File contents = new File( "res/filter");
	...
	private File repoletAContents = new File( input, "archiveA");
	private File repoletBContents = new File( input, "archiveB");
	private File repoletCContents = new File( input, "archiveC");
	
	private File settings = new File( input, "settings");
	private File filters = new File( input, "filters");
	...
	private Collection<Solution> expectedAllMatchingSolutions;
	{
		...
		expectedAllMatchingSolutions.add( NameParser.parseCondensedSolutionName( "com.braintribe.devrock.test:a#1.0.4-pc"));
		expectedAllMatchingSolutions.add( NameParser.parseCondensedSolutionName( "com.braintribe.devrock.test:b#1.0.4-pc"));
		expectedAllMatchingSolutions.add( NameParser.parseCondensedSolutionName( "com.braintribe.devrock.test:c#1.0.4-pc"));
	}
	...
	public void simpleMatchingTest() {
		File settingsToUse = new File( settings, "settings.xml");
		File filterToUse = new File( filters, "simpleFilter.yaml");
		run( settingsToUse, filterToUse, terminalPom, expectedNonLocalMatchingSolutions);		
	}
