...
public abstract class AbstractSubtreeExclusionLab extends AbstractWalkLab {
	private static LauncherShell launcherShell;
	private static File contents = new File( "res/subtreeExclusion/contents");
	...
	public void runTest(String terminalAsString, String [] expectedNames) {
		try {
			Solution terminal = NameParser.parseCondensedSolutionName(terminalAsString);
			
			Collection<Solution> result = test( "runSubTreeExclusionTest", terminal, WalkDenotationTypeExpert.buildCompileWalkDenotationType((ClashStyle.optimistic)), expectedNames,1,0);
