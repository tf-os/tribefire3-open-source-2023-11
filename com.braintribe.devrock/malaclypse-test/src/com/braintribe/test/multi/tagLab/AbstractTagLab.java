...
public abstract class AbstractTagLab extends AbstractWalkLab {
	private static LauncherShell launcherShell;
	private static File contents = new File( "res/tagsLab/contents");
	...
	public void runTest(String terminalAsString, String [] expectedNames, ScopeKind scopeKind, WalkKind walkKind, boolean skipOptional, String tagRule, DependencyScope ... additionalScopes) {
		try {
			...
			if (tagRule != null) {
				walkDenotationType.setTagRule(tagRule);
			}
			Collection<Solution> result = test( "runRangeTest", terminal, walkDenotationType, expectedNames,1,0);
