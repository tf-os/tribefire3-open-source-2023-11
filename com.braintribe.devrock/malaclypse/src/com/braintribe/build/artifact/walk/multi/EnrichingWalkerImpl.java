 ...
 *
 */
public class EnrichingWalkerImpl implements EnrichingConfigurableWalker, WalkNotificationBroadcaster, WalkNotificationListener {
	
	private static Logger log = Logger.getLogger(EnrichingWalkerImpl.class);
	...
	@Override @Configurable @Required
	public void setEnricher(QueueingMultiRepositorySolutionEnricher enricher) {
	...
	public Collection<Solution> walk(String walkScopeId, Solution solution) throws WalkException {		
		...
		try {
			...
			// let the enricher finish..
			long beforeFinalizingEnrichment = System.nanoTime();
			enricher.finalizeEnrichment();
			long afterFinalizingEnrichment = System.nanoTime();
			logElapsedTime("finalizing enrichment on [" + NameParser.buildName( solution) + "]", walkScopeId, beforeFinalizingEnrichment, afterFinalizingEnrichment, true);
