...
import tribefire.extension.messaging.service.test.model.TestUpdateResult;

public class TestUpdateObjectProcessor implements ServiceProcessor<TestUpdateRequest, TestUpdateResult> {
	private static final Logger logger = Logger.getLogger(TestUpdateObjectProcessor.class);
	private PersistenceGmSessionFactory factory;

	@Override
	public TestUpdateResult process(ServiceRequestContext requestContext, TestUpdateRequest request) {
		// This is a stub processor to fulfill the requirements for integration testing
		logger.info("We reached test destination processor!");
		...
		if (result == null) {
			throw new IllegalArgumentException("Test Object with id: " + request.getRelatedObjId() + " not Found!");
		}
		result.setName(request.getNewName());
		session.commit();
		return TestUpdateResult.T.create();
	}
