...
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.CSS_IMG_REFERENCES;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.CSS_IMG_URL_PATTERN;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.DATE_PATTERN_ANSI_C;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.DATE_PATTERN_HTTP_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.DATE_PATTERN_RFC_1036;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.DATE_PATTERN_RFC_1123;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.DEFAULT_LOCALE_US;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.EXT_CSS;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.EXT_JS;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.EXT_JSON;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.MIME_CSS;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.MIME_JS;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.MIME_JSON;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.MIME_OCTET_STREAM;
 ...
 * @version 1.0
 */
// TODO: check migration from 2.2.1 - changed completely
public final class Utils {
	...
	/**
	 * @param string
	 *            string representation of a int which is to be parsed and read from
	 * @param defaultValue
	 *            in case parsing fails or string is null, returns this default value
	...
	/**
	 * @param string
	 *            string representation of a long which is to be parsed and read from
	 * @param defaultValue
	 *            in case parsing fails or string is null, returns this default value
	...
	/**
	 * @param string
	 *            string representation of a boolean (true or false) which is to be parsed and read from
	 * @param defaultValue
	 *            in case string is null or does not contain valid boolean, returns this default value
	 ...
	 *            string
	 * @param defaultValue
	 *            in case string is null or empty
	 ...
	 */
	public static String detectExtension(String requestURI) { // !TODO case sensitivity?
																// http://server/context/path/a.CSS
		return requestURI.substring(requestURI.lastIndexOf("."));
	}

	/**
	 * @param filePath
	 *            - path of the file, whose mime is to be detected
	...
	/**
	 * @param extensionOrFile
	 *            - .js or .css etc. of full file path in case of image files
	 ...
	 *
	 * @param resourceRealPath
	 *            - file path, whose has to be calculated
	...
	/**
	 * Split multiple resources with comma eg. if URL is http://server/context/js/a,b,c.js then a.js, b.js and c.js have
	 * to be processed and merged together.
	 * <p/>
	...
	public static List<String> findResourcesToMerge(String contextPath, String requestURI) {
		...
		requestURI = requestURI.replaceFirst(contextPath, "");// .replace(extension, "");//remove the context path &
																// ext. will become /path/subpath/a,b,/anotherpath/c
	 ...
	 *            - list of resources
	 * @param requestETag
	 *            - request ETag from If-None-Match header
	 * @param actualETag
	 *            - current ETag of a resource (can be null)
	 * @param servletContext
	 *            - servlet context
		...
		if (realPath.endsWith(EXT_CSS)) { // check if any image references by this css has been modified or not
			...
			if (referencedImages != null) {
				...
				try {
					...
					LOGGER.warn("Failed to read/touch " + realPath, ex);
				}
			}
		}
	 ...
	 *
	 * @param path
	 *            path whose parent path has to be returned
	...
	/**
	 * A utility method to build headers map from the string. Multiple headers in the string would be separated by | and
	 * the header key and value/s would be separated by :
	 *
	 * Example String: X-Requested-By: applicationOne | Accepts: *.* | Custom-Header: Custom Value
	 *
	 * @param headerString
	 *            the string to parse and build the map from. Ex. HeaderName: value | HeaderName: value
