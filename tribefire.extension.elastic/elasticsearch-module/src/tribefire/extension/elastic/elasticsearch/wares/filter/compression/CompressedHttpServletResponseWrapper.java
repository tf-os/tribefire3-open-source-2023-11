...
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_ACCEPT_ENCODING_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_CACHE_CONTROL_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_CONTENT_ENCODING_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_CONTENT_LENGTH_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_CONTENT_TYPE_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_ETAG_HEADER;
import static tribefire.extension.elastic.elasticsearch.wares.filter.common.Constants.HTTP_VARY_HEADER;
...
import tribefire.extension.elastic.elasticsearch.wares.filter.common.IgnoreAcceptContext;
import tribefire.extension.elastic.elasticsearch.wares.filter.common.WebUtilitiesResponseWrapper;
...
public class CompressedHttpServletResponseWrapper extends WebUtilitiesResponseWrapper {
	...
	private int threshold = Constants.DEFAULT_COMPRESSION_SIZE_THRESHOLD;
	 ...
	 * Logger
	 */
	protected static Logger LOGGER = Logger.getLogger(CompressedHttpServletResponseWrapper.class);

	public CompressedHttpServletResponseWrapper(HttpServletResponse httpResponse, EncodedStreamsFactory encodedStreamsFactory, String contentEncoding,
			int threshold, IgnoreAcceptContext ignoreAcceptContext) {
		super(httpResponse);
	...
	public PrintWriter getWriter() throws IOException {
		...
		if (printWriter == null) {
			printWriter = new PrintWriter(new OutputStreamWriter(getCompressedServletOutputStream(), getCharacterEncoding()), true);
		}
		return printWriter;
	}
	...
	private CompressedServletOutputStream getCompressedServletOutputStream() throws IOException {
		if (compressingStream == null) {
			compressingStream = new CompressedServletOutputStream(httpResponse.getOutputStream(), encodedStreamsFactory, this, threshold);
		}
	...
	private boolean mustNotCompress() {
		...
		if (savedContentLengthSet && savedContentLength < threshold) {
			LOGGER.trace("No Compression: Already set content length " + savedContentLength + " less than threshold " + threshold);
			return true;
		}
		if (noTransformSet) {
