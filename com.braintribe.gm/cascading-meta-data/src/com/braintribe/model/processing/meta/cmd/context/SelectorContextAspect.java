...
/**
 * Represents some type of information that may be contained in the {@link SelectorContext}. There are some pre-defined
 * types which are used by the resolver itself, but in general it is up to the client (user of the resolver) to create
 * his own custom aspects (by extending this interface).
 * <p>
 ...
 * <p>
 * One may also annotate a given aspect with a scope, indicating how stable (or likely to be changed) a given aspect is,
 * using the {@link ScopedAspect} annotation. It is recommended to annotate the aspect with the most stable scope
 * possible, since this information is used for caching.
 * 
 * @param <T>
