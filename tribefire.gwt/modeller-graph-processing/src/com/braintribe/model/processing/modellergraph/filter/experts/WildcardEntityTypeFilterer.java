...
public class WildcardEntityTypeFilterer extends WildcardFilterer<WildcardEntityTypeFilter> {
	@Override
	protected String[] getValues(CondensedRelationshipContext relationshipContext) {
		return new String[]{relationshipContext.getRelationship().getToType().getGmType().getTypeSignature()};
	}
	
	@Override
	protected String getValue(Relationship relationship) {
		return relationship.getToType().getTypeSignature();
	}
}
