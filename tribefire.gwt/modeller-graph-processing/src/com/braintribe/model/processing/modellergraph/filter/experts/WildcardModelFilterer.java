...
public class WildcardModelFilterer extends WildcardFilterer<WildcardArtifactBindingFilter> {
	@Override
	protected String[] getValues(CondensedRelationshipContext relationshipContext) {
		return new String[]{relationshipContext.getRelationship().getToType().getModel().getModel().getName()};
	}
	
	@Override
	protected String getValue(Relationship relationship) {
		return relationship.getToType().getDeclaringModel().getName();
	}
}
