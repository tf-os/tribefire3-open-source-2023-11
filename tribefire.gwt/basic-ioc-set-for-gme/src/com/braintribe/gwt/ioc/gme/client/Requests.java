...
class Requests {
	...
	protected static Supplier<CurrentUserInformationRequest> userRequest = new SessionScopedBeanProvider<CurrentUserInformationRequest>() {
		@Override
		public CurrentUserInformationRequest create() throws Exception {
			CurrentUserInformationRequest bean = CurrentUserInformationRequest.T.create();
			return bean;
		}
	};

	private static Supplier<AvailableAccessesRequest> accessesRequest = new SessionScopedBeanProvider<AvailableAccessesRequest>() {
		@Override
		public AvailableAccessesRequest create() throws Exception {
			AvailableAccessesRequest bean = AvailableAccessesRequest.T.create();
			bean.setIncludeHardwired(Runtime.useHardwiredAccesses);
			return bean;
		}
	};
	...
	protected static Supplier<BootstrappingRequest> bootstrappingRequest = new SessionScopedBeanProvider<BootstrappingRequest>() {
		...
		public BootstrappingRequest create() throws Exception {
			...
			if (Runtime.handleInitializationUI) {
				bean.setAccessesRequest(accessesRequest.get());
				bean.setUserRequest(userRequest.get());
			}
