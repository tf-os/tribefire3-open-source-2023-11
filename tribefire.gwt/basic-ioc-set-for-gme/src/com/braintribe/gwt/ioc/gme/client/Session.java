...
class Session {
	...
	protected static Supplier<ModelEnvironmentDrivenGmSession> rawSession = new SessionScopedBeanProvider<ModelEnvironmentDrivenGmSession>() {
		@Override
		public ModelEnvironmentDrivenGmSession create() throws Exception {
			ModelEnvironmentDrivenGmSession bean = publish(abstractSession.get());
			return bean;
		}
	};
	...
	protected static Supplier<AccessServiceGwtPersistenceGmSession> persistenceSession = new SessionScopedBeanProvider<AccessServiceGwtPersistenceGmSession>() {
		...
		public AccessServiceGwtPersistenceGmSession create() throws Exception {
			...
			GxtReasonErrorDialog.setGmSession(bean);
			return bean;
		}
	};

	protected static Supplier<AccessServiceGwtPersistenceGmSession> userSession = new SessionScopedBeanProvider<AccessServiceGwtPersistenceGmSession>() {
		@Override
		public AccessServiceGwtPersistenceGmSession create() throws Exception {
			AccessServiceGwtPersistenceGmSession bean = publish(abstractSession.get());
			return bean;
	...
	/*private static Supplier<GwtSessionResourceSupport> abstractResourceAccess = new PrototypeBeanProvider<GwtSessionResourceSupport>() {
		@Override
		public GwtSessionResourceSupport create() throws Exception {
			GwtSessionResourceSupport bean = new GwtSessionResourceSupport();
			bean.setSessionIdProvider(Providers.sessionIdProvider.get());
			bean.setStreamBaseUrl(Runtime.tribefireServicesUrl.get() + "streaming");
			return bean;
		}
	};*/
