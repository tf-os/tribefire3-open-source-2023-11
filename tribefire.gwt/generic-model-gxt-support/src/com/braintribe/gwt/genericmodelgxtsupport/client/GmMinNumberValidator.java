...
public class GmMinNumberValidator<N extends Number> extends MinNumberValidator<N> {
	...
	public GmMinNumberValidator(N minNumber) {
		super(minNumber);
	}
	...
	public List<EditorError> validate(Editor<N> field, N value) {
		...
		if (value.doubleValue() < minNumber.doubleValue()) {
			if (validation != null) {
	        ...
	        if (field instanceof ValueBaseField) {
	          ValueBaseField<N> vbf = (ValueBaseField<N>) field;
	          PropertyEditor<N> pe = vbf.getPropertyEditor();
	          minText = pe.render(minNumber);
	        } else {
	          minText = minNumber.toString();
	        }
			
			tip = prepareToolTip(getMessages().numberMinText(minText));
			tip.showAtHoverElement();			
			return null;
	    } else {
			if (tip != null && tip.isVisible())
