...
public class GmEmptyValidator<T> extends EmptyValidator<T>{
	...
	public List<EditorError> validate(Editor<T> editor, T value) {		
	    if (value == null || "".equals(value)) {	    	
			...
			tip = prepareToolTip(getMessages().blankText());
			tip.showAtHoverElement();			
			return null;
			
			//List<EditorError> errors = new ArrayList<EditorError>();
		    //errors.add(new DefaultEditorError(editor, getErrorMessage(), ""));
		    //return errors;
	    } else {
			if (tip != null && tip.isVisible())
				tip.hide();	    	
